...
int mbedtls_ssl_set_cid(mbedtls_ssl_context *ssl,
	...
	if (own_cid_len != ssl->conf->cid_len) {
		MBEDTLS_SSL_DEBUG_MSG(3, ("CID length %u does not match CID length %u in config", (unsigned)own_cid_len, (unsigned)ssl->conf->cid_len));
		return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
	}
...
static void handle_buffer_resizing(mbedtls_ssl_context *ssl, int downsizing,
	...
	if (ssl->in_buf != NULL) {
		...
		if (downsizing ? ssl->in_buf_len > in_buf_new_len && ssl->in_left < in_buf_new_len : ssl->in_buf_len < in_buf_new_len) {
			if (resize_buffer(&ssl->in_buf, in_buf_new_len, &ssl->in_buf_len) != 0) {
				MBEDTLS_SSL_DEBUG_MSG(1, ("input buffer resizing failed - out of memory"));
			} else {
				MBEDTLS_SSL_DEBUG_MSG(2, ("Reallocating in_buf to %" MBEDTLS_PRINTF_SIZET, in_buf_new_len));
				modified = 1;
			}
		}
	}
	...
	if (ssl->out_buf != NULL) {
		...
		if (downsizing ? ssl->out_buf_len > out_buf_new_len && ssl->out_left < out_buf_new_len : ssl->out_buf_len < out_buf_new_len) {
			if (resize_buffer(&ssl->out_buf, out_buf_new_len, &ssl->out_buf_len) != 0) {
				MBEDTLS_SSL_DEBUG_MSG(1, ("output buffer resizing failed - out of memory"));
			} else {
				MBEDTLS_SSL_DEBUG_MSG(2, ("Reallocating out_buf to %" MBEDTLS_PRINTF_SIZET, out_buf_new_len));
				modified = 1;
			}
		}
	}
...
static int ssl_handshake_init(mbedtls_ssl_context *ssl) {
	...
	if (mbedtls_ssl_conf_is_tls12_only(ssl->conf) &&
		...
		MBEDTLS_STATIC_ASSERT(MBEDTLS_SSL_MAX_SIG_ALG_LIST_LEN <= (SIZE_MAX - (2 * sizeof(uint16_t))),
...
int mbedtls_ssl_set_session(mbedtls_ssl_context *ssl, const mbedtls_ssl_session *session) {
	...
	if (session->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
		...
		if (mbedtls_ssl_validate_ciphersuite(
			...
			MBEDTLS_SSL_DEBUG_MSG(4, ("%d is not a valid TLS 1.3 ciphersuite.", session->ciphersuite));
			return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
		}
#else
		/*
...
void mbedtls_ssl_conf_tls13_enable_signal_new_session_tickets(
	...
	conf->session_tickets |= (signal_new_session_tickets != 0) << MBEDTLS_SSL_SESSION_TICKETS_TLS1_3_BIT;
}
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 */
#endif /* MBEDTLS_SSL_CLI_C */
...
#define SSL_SERIALIZED_SESSION_CONFIG_BITFLAG                                                                  \
	((uint16_t)((SSL_SERIALIZED_SESSION_CONFIG_TIME << SSL_SERIALIZED_SESSION_CONFIG_TIME_BIT) |               \
			...
			(SSL_SERIALIZED_SESSION_CONFIG_EARLY_DATA << SSL_SERIALIZED_SESSION_CONFIG_EARLY_DATA_BIT) |       \
			(SSL_SERIALIZED_SESSION_CONFIG_RECORD_SIZE << SSL_SERIALIZED_SESSION_CONFIG_RECORD_SIZE_BIT) |     \
...
#if defined(MBEDTLS_SSL_CLI_C)
	if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT) {
		MBEDTLS_SSL_DEBUG_MSG(2, ("client state: %s", mbedtls_ssl_states_str((mbedtls_ssl_states)ssl->state)));
...
#define SSL_SERIALIZED_CONTEXT_CONFIG_BITFLAG                                                                                \
	((uint32_t)((SSL_SERIALIZED_CONTEXT_CONFIG_DTLS_CONNECTION_ID << SSL_SERIALIZED_CONTEXT_CONFIG_DTLS_CONNECTION_ID_BIT) | \
...
int mbedtls_ssl_parse_sig_alg_ext(mbedtls_ssl_context *ssl,
	...
	while (p < supported_sig_algs_end) {
		...
		p += 2;
		MBEDTLS_SSL_DEBUG_MSG(4, ("received signature algorithm: 0x%x %s", sig_alg, mbedtls_ssl_sig_alg_to_str(sig_alg)));
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
		if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_2 &&
		...
		MBEDTLS_SSL_DEBUG_MSG(4, ("valid signature algorithm: %s", mbedtls_ssl_sig_alg_to_str(sig_alg)));
...
int mbedtls_ssl_write_certificate(mbedtls_ssl_context *ssl) {
	...
	while (crt != NULL) {
		...
		if (n > MBEDTLS_SSL_OUT_CONTENT_LEN - 3 - i) {
			MBEDTLS_SSL_DEBUG_MSG(1, ("certificate too large, %" MBEDTLS_PRINTF_SIZET " > %" MBEDTLS_PRINTF_SIZET, i + 3 + n, (size_t)MBEDTLS_SSL_OUT_CONTENT_LEN));
			return MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL;
		}
...
static int ssl_tls12_populate_transform(mbedtls_ssl_transform *transform,
	...
	if (mac_alg == 0) {
		MBEDTLS_SSL_DEBUG_MSG(1, ("mbedtls_md_psa_alg_from_type for %u not found", (unsigned)ciphersuite_info->mac));
		return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
	}
#else
	...
	MBEDTLS_SSL_DEBUG_MSG(3, ("keylen: %u, minlen: %u, ivlen: %u, maclen: %u", (unsigned)keylen, (unsigned)transform->minlen, (unsigned)transform->ivlen, (unsigned)transform->maclen));
...
int mbedtls_ssl_write_sig_alg_ext(mbedtls_ssl_context *ssl, unsigned char *buf,
	...
	*out_len = 0;
	...
	for (; *sig_alg != MBEDTLS_TLS1_3_SIG_NONE; sig_alg++) {
		MBEDTLS_SSL_DEBUG_MSG(3, ("got signature scheme [%x] %s", *sig_alg, mbedtls_ssl_sig_alg_to_str(*sig_alg)));
		...
		p += 2;
		MBEDTLS_SSL_DEBUG_MSG(3, ("sent signature scheme [%x] %s", *sig_alg, mbedtls_ssl_sig_alg_to_str(*sig_alg)));
	}
