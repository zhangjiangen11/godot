...
void HeightFieldShape::RestoreBinaryState(StreamIn &inStream) {
	...
	Shape::RestoreBinaryState(inStream);// 8

	inStream.Read(mOffset);// 20
	inStream.Read(mScale);// 32
	inStream.Read(mSampleCount); // 40
	inStream.Read(mBlockSize); // 44
	inStream.Read(mBitsPerSample); // 45
	inStream.Read(mMinSample);//47
	inStream.Read(mMaxSample);//49
	...
	// mMaterials.reserve(PhysicsMaterialList::size_type(1) << (32 - mNumBitsPerMaterialIndex));
