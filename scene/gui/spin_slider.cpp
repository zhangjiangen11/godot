...
String SpinSlider::get_tooltip(const Point2 &p_pos) const {
...
Size2 SpinSlider::get_minimum_size() const {
	Ref<StyleBox> sb = theme_cache.style_normal;
...
String SpinSlider::get_text_value() const {
...
void SpinSlider::gui_input(const Ref<InputEvent> &p_event) {
...
void SpinSlider::_grabber_gui_input(const Ref<InputEvent> &p_event) {
	if (read_only) {
		return;
	}
...
void SpinSlider::_value_input_gui_input(const Ref<InputEvent> &p_event) {
	Ref<InputEventKey> k = p_event;
...
void SpinSlider::_update_value_input_stylebox() {
	if (!value_input) {
		return;
	}
	...
	// default margins.
	Ref<StyleBox> stylebox = theme_cache.style_normal;
	// TODO: Handle this internally instead of relying on the theme.
	int margin = get_theme_constant(get_label().is_empty() ? SNAME("line_edit_margin_empty") : SNAME("line_edit_margin"), SNAME("SpinSlider"));
...
void SpinSlider::_draw_spin_slider() {
	updown_offset = -1;
	...
	Ref<StyleBox> sb = read_only ? theme_cache.style_readonly : theme_cache.style_normal;
	if (!flat) {
	...
	Color lc = read_only ? theme_cache.read_only_label_color : theme_cache.font_color;

	if (flat && !label.is_empty()) {
		Ref<StyleBox> label_bg = theme_cache.label_bg;
		if (rtl) {
	...
	if (has_focus(true)) {
		Ref<StyleBox> focus = theme_cache.style_focus;
	...
	if (control_state != CONTROL_STATE_HIDE) {
		if (editing_integer && control_state == CONTROL_STATE_DEFAULT) {
			...
			const int width = size.width - sb->get_minimum_size().width - theme_cache.grabber_w;
			...
			c.a = 0.2;
			draw_rect(Rect2(ofs, svofs + 1, width, 2), c);

			// Draw the horizontal slider's filled part on the left.
			const int gofs = get_as_ratio() * width;
			c.a = 0.45;
			draw_rect(Rect2(ofs, svofs + 1, gofs, 2), c);

			// Draw the horizontal slider's grabber.
			c.a = 0.9;
			const Rect2 grabber_rect = Rect2(ofs + gofs, svofs, theme_cache.grabber_w, 4);
...
void SpinSlider::_notification(int p_what) {
	switch (p_what) {
		case NOTIFICATION_ENTER_TREE: {
			grabbing_spinner_speed = editing_integer ? integer_drag_speed : float_drag_speed;
...
LineEdit *SpinSlider::get_line_edit() {
	_ensure_value_input();
	return value_input;
}

void SpinSlider::set_control_state(ControlState p_state) {
	control_state = p_state;
	queue_redraw();
}

SpinSlider::ControlState SpinSlider::get_control_state() const {
	return control_state;
}

#ifndef DISABLE_DEPRECATED
void SpinSlider::set_hide_slider(bool p_hide) {
	set_control_state(p_hide ? CONTROL_STATE_HIDE : CONTROL_STATE_DEFAULT);
}

bool SpinSlider::is_hiding_slider() const {
	return control_state == CONTROL_STATE_HIDE;
}
#endif

void SpinSlider::set_editing_integer(bool p_editing_integer) {
...
bool SpinSlider::is_editing_integer() const {
	return editing_integer;
}

void SpinSlider::set_label(const String &p_label) {
	label = p_label;
	queue_redraw();
}

String SpinSlider::get_label() const {
	return label;
}

void SpinSlider::set_suffix(const String &p_suffix) {
	suffix = p_suffix;
	queue_redraw();
}

String SpinSlider::get_suffix() const {
	return suffix;
}

void SpinSlider::_evaluate_input_text() {
...
void SpinSlider::_value_input_submitted(const String &p_text) {
	value_input_closed_frame = Engine::get_singleton()->get_frames_drawn();
	if (value_input) {
		value_input_focus_visible = value_input->has_focus(true);
		value_input->hide();
...
void SpinSlider::_value_input_hidden() {
	_evaluate_input_text();
	value_input_closed_frame = Engine::get_singleton()->get_frames_drawn();
}

void SpinSlider::_value_focus_exited() {
...
void SpinSlider::_grabber_mouse_entered() {
	mouse_over_grabber = true;
	queue_redraw();
}

void SpinSlider::_grabber_mouse_exited() {
	mouse_over_grabber = false;
	queue_redraw();
}

void SpinSlider::set_read_only(bool p_enable) {
	read_only = p_enable;
...
bool SpinSlider::is_read_only() const {
	return read_only;
}

void SpinSlider::set_flat(bool p_enable) {
	flat = p_enable;
	queue_redraw();
}

bool SpinSlider::is_flat() const {
	return flat;
}

bool SpinSlider::is_grabbing() const {
	return grabbing_grabber || grabbing_spinner;
}

void SpinSlider::_focus_entered(bool p_hide_focus) {
	if (read_only) {
		return;
	}
...
void SpinSlider::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_label", "label"), &SpinSlider::set_label);
	ClassDB::bind_method(D_METHOD("get_label"), &SpinSlider::get_label);

	ClassDB::bind_method(D_METHOD("set_suffix", "suffix"), &SpinSlider::set_suffix);
	ClassDB::bind_method(D_METHOD("get_suffix"), &SpinSlider::get_suffix);

	ClassDB::bind_method(D_METHOD("set_read_only", "read_only"), &SpinSlider::set_read_only);
	ClassDB::bind_method(D_METHOD("is_read_only"), &SpinSlider::is_read_only);

	ClassDB::bind_method(D_METHOD("set_flat", "flat"), &SpinSlider::set_flat);
	ClassDB::bind_method(D_METHOD("is_flat"), &SpinSlider::is_flat);

	ClassDB::bind_method(D_METHOD("set_control_state", "state"), &SpinSlider::set_control_state);
	ClassDB::bind_method(D_METHOD("get_control_state"), &SpinSlider::get_control_state);
#ifndef DISABLE_DEPRECATED
	ClassDB::bind_method(D_METHOD("set_hide_slider", "hide_slider"), &SpinSlider::set_hide_slider);
	ClassDB::bind_method(D_METHOD("is_hiding_slider"), &SpinSlider::is_hiding_slider);
#endif

	ClassDB::bind_method(D_METHOD("set_editing_integer", "editing_integer"), &SpinSlider::set_editing_integer);
	ClassDB::bind_method(D_METHOD("is_editing_integer"), &SpinSlider::is_editing_integer);
	...
	BIND_THEME_ITEM(Theme::DATA_TYPE_CONSTANT, SpinSlider, grabber_w);

	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_ICON, SpinSlider, updown_icon, "updown");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_ICON, SpinSlider, updown_disabled_icon, "updown_disabled");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_ICON, SpinSlider, grabber, "grabber");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_ICON, SpinSlider, grabber_highlight, "grabber_highlight");

	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_STYLEBOX, SpinSlider, label_bg, "label_bg");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_STYLEBOX, SpinSlider, style_normal, "normal");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_STYLEBOX, SpinSlider, style_focus, "focus");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_STYLEBOX, SpinSlider, style_readonly, "read_only");

	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_COLOR, SpinSlider, font_color, "font_color");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_COLOR, SpinSlider, font_uneditable_color, "uneditable_color");
	BIND_THEME_ITEM_CUSTOM(Theme::DATA_TYPE_COLOR, SpinSlider, read_only_label_color, "only_label_color");

	BIND_THEME_ITEM(Theme::DATA_TYPE_FONT, SpinSlider, font);
	BIND_THEME_ITEM(Theme::DATA_TYPE_FONT_SIZE, SpinSlider, font_size);
}

void SpinSlider::_ensure_value_input() {
	if (value_input) {
		return;
	}
	...
	value_input->connect(SceneStringName(hidden), callable_mp(this, &SpinSlider::_value_input_hidden));
	value_input->connect(SceneStringName(text_submitted), callable_mp(this, &SpinSlider::_value_input_submitted));
	value_input->connect(SceneStringName(focus_exited), callable_mp(this, &SpinSlider::_value_focus_exited));
	value_input->connect(SceneStringName(gui_input), callable_mp(this, &SpinSlider::_value_input_gui_input));
...
SpinSlider::SpinSlider() {
	...
	grabber->connect(SceneStringName(mouse_entered), callable_mp(this, &SpinSlider::_grabber_mouse_entered));
	grabber->connect(SceneStringName(mouse_exited), callable_mp(this, &SpinSlider::_grabber_mouse_exited));
	grabber->connect(SceneStringName(gui_input), callable_mp(this, &SpinSlider::_grabber_gui_input));
}
