...
void MDecal::set_texture(DecalTexture p_type, const Ref<Texture2D> &p_texture) {
	...
	RSS->decal_set_texture(decal, DecalTexture(p_type), texture_rid);
...
void MDecal::set_emission_energy(real_t p_energy) {
	...
	RSS->decal_set_emission_energy(decal, emission_energy);
}
...
void MDecal::set_albedo_mix(real_t p_mix) {
	albedo_mix = p_mix;
	RSS->decal_set_albedo_mix(decal, albedo_mix);
}
...
void MDecal::set_upper_fade(real_t p_fade) {
	...
	RSS->decal_set_fade(decal, upper_fade, lower_fade);
}
...
void MDecal::set_normal_fade(real_t p_fade) {
	normal_fade = p_fade;
	RSS->decal_set_normal_fade(decal, normal_fade);
}
...
void MDecal::set_modulate(Color p_modulate) {
	modulate = p_modulate;
	RSS->decal_set_modulate(decal, p_modulate);
}
...
void MDecal::set_enable_distance_fade(bool p_enable) {
	...
	RSS->decal_set_distance_fade(decal, distance_fade_enabled, distance_fade_begin, distance_fade_length);
...
void MDecal::set_cull_mask(uint32_t p_layers) {
	cull_mask = p_layers;
	RSS->decal_set_cull_mask(decal, cull_mask);
}
