...
float MSmooth::get_height(uint32_t x, uint32_t y) {
	...
	if (mode == 0) {
		float total = 1;
		float toh = h;
		if (grid->has_pixel(x - 1, y - 1)) {
			float oh = grid->get_height_by_pixel(x - 1, y - 1);
			if (!std::isnan(oh)) {
				toh += oh;
				total += 1;
			}
		}
		if (grid->has_pixel(x, y - 1)) {
			float oh = grid->get_height_by_pixel(x, y - 1);
			if (!std::isnan(oh)) {
				toh += oh;
				total += 1;
			}
		...
		if (grid->has_pixel(x - 1, y)) {
			float oh = grid->get_height_by_pixel(x - 1, y);
			if (!std::isnan(oh)) {
				toh += oh;
				total += 1;
			}
		...
		if (grid->has_pixel(x, y - 1)) {
			float oh = grid->get_height_by_pixel(x, y - 1) * 2.0;
			if (!std::isnan(oh)) {
				toh += oh;
				total += 2;
			}
		...
		if (grid->has_pixel(x - 1, y)) {
			float oh = grid->get_height_by_pixel(x - 1, y) * 2.0;
			if (!std::isnan(oh)) {
				toh += oh;
				total += 2;
			}
