...
void MAssetMesh::update_lod(int lod){
    ...
    for(InstanceData& data : instance_data){
        ...
        if(mesh_rid.is_valid() && !data.mesh_rid.is_valid()){ // create
            data.instance_rid = RSS->instance_create();
            instance_count++;
            RSS->instance_set_scenario(data.instance_rid,get_world_3d()->get_scenario());
            RSS->instance_set_transform(data.instance_rid,get_global_transform() * data.local_transform);
        ...
        RSS->instance_set_base(data.instance_rid,mesh_rid);
        ...
        if(mmesh.is_valid() && mmesh->has_material_override() && data.material_set_id >= 0){
            ...
            for(int i=0; i < material_rids.size(); i++){
                RSS->instance_set_surface_override_material(data.instance_rid,i,material_rids[i]);
            }
