...
void MAssetTable::save() {
	...
	make_assets_dir();
	Error err = ResourceSaver::save(asset_table_singelton, asset_table_path);
...
void MAssetTable::_increase_collection_buffer_size(int q) {
	...
	Error err = collections.resize(lsize + q);
	ERR_FAIL_COND_MSG(err != OK, "Can't increase collection item buffer size, possible fragmentation error!");
...
int MAssetTable::tag_add(const String &_name) {
	ERR_FAIL_COND_V_MSG(tag_names.has(_name), -1, "Tag \"" + _name + "\" already exist");
	for (int i = 0; i < M_MAX_TAG; i++) {
		if (tag_names[i].is_empty()) {
			tag_names.set(i, _name);
			return i;
		}
	}
...
int32_t MAssetTable::mesh_item_get_stop_lod(int item_id) {
	...
	for (int i = 0; i < MAX_MESH_LOD; i++) {
		...
		if (FileAccess::exists(mstop_path)) {
			return i;
		}
	}
	return -1;
...
TypedArray<MMesh> MAssetTable::mesh_item_meshes_no_replace(int item_id) {
	...
	for (int32_t id : ids) {
		if (id < 0) {
			...
			out.push_back(ResourceLoader::load(MHlod::get_mesh_path(id)));
		}
	}
	return out;
}
...
TypedArray<MMesh> MAssetTable::mesh_item_meshes(int item_id) {
	...
	for (int i = 0; i < item_ids.size(); i++) {
		...
		Ref<MMesh> _m = ResourceLoader::load(MHlod::get_mesh_path(item_ids[i]));
		out.push_back(_m);
	}
	return out;
}
...
int32_t MAssetTable::mesh_join_get_stop_lod(int item_id) {
	...
	for (int i = 0; i < MAX_MESH_LOD; i++) {
		...
		if (FileAccess::exists(stop_path)) {
			return i;
		}
	}
	return -1;
...
TypedArray<MMesh> MAssetTable::mesh_join_meshes(int item_id) {
	...
	for (int32_t id : ids) {
		if (id != -1) {
			...
			out.push_back(ResourceLoader::load(path));
		} else {
			out.push_back(Ref<MMesh>());
		}
	}
...
void MAssetTable::load_import_info() {
	if (FileAccess::exists(import_info_path)) {
