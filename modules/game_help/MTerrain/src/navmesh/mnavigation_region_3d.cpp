...
void MNavigationRegion3D::_update_navmesh(Vector3 _cam_pos) {
	...
	uint32_t right = bottom_right.x >= int32_t(grid->grid_pixel_region.right) ? grid->grid_pixel_region.right - 1 : bottom_right.x;
	uint32_t bottom = bottom_right.y >= int32_t(grid->grid_pixel_region.bottom) ? grid->grid_pixel_region.bottom - 1 : bottom_right.y;
	...
	for (uint32_t y = top; y < bottom; y += l) {
		for (uint32_t x = left; x < right; x += l) {
			...
			int end_y = y + l;
			int end_x = x + l;
			for (int32_t j = y; j < end_y; j++) {
				for (int32_t i = x; i < end_x; i++) {
