...
void MGrass::create_grass_chunk(int grid_index, MGrassChunk *grass_chunk) {
	...
	for (int k = 0; k < pixel_regions.size(); k++) {
		...
		if (meshes.is_null() || meshes->get_mesh_rid(lod) == RID() || int32_t(count) < min_grass_cutoff) {
...
void MGrass::draw_grass(Vector3 brush_pos, real_t radius, bool add) {
	...
	if (px_pos.x < 0 || px_pos.y < 0 || px_pos.x > int32_t(width) || px_pos.y > int32_t(height)) {
		return;
	}
...
void MGrass::clear_grass_sublayer_aabb(AABB aabb) {
	...
	end.x = end.x < int32_t(width) ? end.x : int32_t(width) - 1;
	end.y = end.y < int32_t(height) ? end.y : int32_t(height) - 1;
...
void MGrass::update_physics(Vector3 cam_pos) {
	...
	int update_count = 0;
	for (int32_t y = physics_search_bound.top; y <= physics_search_bound.bottom; y++) {
		for (int32_t x = physics_search_bound.left; x <= physics_search_bound.right; x++) {
...
bool MGrass::_set(const StringName &p_name, const Variant &p_value) {
	if (p_name.begins_with("Material_LOD_")) {
		PackedStringArray s = p_name.str().split("_");
		int index = s[2].to_int();
		if (index > materials.size() - 1) {
			return false;
		}
