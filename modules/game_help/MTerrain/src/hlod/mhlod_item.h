...
struct MHLodItemMesh {
	...
	_FORCE_INLINE_ bool has_cache() const {
		return mesh.is_valid() || ResourceCache::has(M_GET_MESH_PATH(mesh_id));
	}
	_FORCE_INLINE_ void load() {
		...
		if (ResourceCache::has(mpath)) {
			mesh = ResourceCache::get_ref(mpath);
		} else {
...
struct MHLodItemDecal {
	...
	Ref<MDecal> decal;
	_FORCE_INLINE_ bool has_cache() const {
		return decal.is_valid() || ResourceCache::has(M_GET_DECAL_PATH(decal_id));
	}
	_FORCE_INLINE_ void load() {
		ERR_FAIL_COND_MSG(decal.is_valid(), "Decal valid on load!");
		String dpath = M_GET_DECAL_PATH(decal_id);
		if (ResourceCache::has(dpath)) {
			decal = ResourceCache::get_ref(dpath);
		} else {
	...
	_FORCE_INLINE_ void unload() {
		if (shapes_list.has(param)) {
			...
			if (shapes_list.getptr(param)->user_count == 0) {
				PhysicsServer3D::get_singleton()->free(shapes_list[param].rid);
...
struct MHLodItemCollisionComplex {
	...
	Ref<Shape3D> shape;
	_FORCE_INLINE_ bool has_cache() const {
		return shape.is_valid() || ResourceCache::has(M_GET_COLLISION_PATH(id));
	}
	_FORCE_INLINE_ void load() {
		ERR_FAIL_COND_MSG(shape.is_valid(), "Load on valid shape MHLodItemCollisionComplex");
		String spath = M_GET_COLLISION_PATH(id);
		if (ResourceCache::has(spath)) {
			shape = ResourceCache::get_ref(spath);
		} else {
...
struct MHLodItemLight { // No more member or increase item size
	...
	_FORCE_INLINE_ void load() {
		...
		switch (type) {
			case Type::SPOT:
				light = RSS->spot_light_create();
				break;
			case Type::OMNI:
				light = RSS->omni_light_create();
				// Only two mode
		...
		// color
		RSS->light_set_color(light, Color(red.get_float(), green.get_float(), blue.get_float()));
		// cull mask
		RSS->light_set_cull_mask(light, cull_mask);
		// booleans
		RSS->light_set_shadow(light, (bool)shadow_enabled);
		RSS->light_set_reverse_cull_face_mode(light, (bool)shadow_reverse_cull_face);
		RSS->light_set_negative(light, (bool)negetive);
		// Params
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_ENERGY, energy.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_INDIRECT_ENERGY, light_indirect_energy.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_VOLUMETRIC_FOG_ENERGY, light_volumetric_fog_energy.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SIZE, size.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SPECULAR, specular.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_RANGE, range.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_ATTENUATION, attenuation.get_float());
		// spot light param
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SPOT_ANGLE, spot_angle.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SPOT_ATTENUATION, spot_attenuation.get_float());
		// shadow
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SHADOW_BIAS, shadow_bias.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SHADOW_NORMAL_BIAS, shadow_normal_bias.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SHADOW_OPACITY, shadow_opacity.get_float());
		RSS->light_set_param(light, RenderingServer::LIGHT_PARAM_SHADOW_BLUR, shadow_blur.get_float());
		// distance fade
		RSS->light_set_distance_fade(light, (bool)distance_fade_enabled, distance_fade_begin, distance_fade_shadow, distance_fade_length);
	...
	_FORCE_INLINE_ void set_data(const PackedByteArray &d, int head) {
		ERR_FAIL_COND(d.size() < int64_t(sizeof(MHLodItemLight) + head));
...
struct MHLodItemPackedScene {
	...
	_FORCE_INLINE_ bool has_cache() const {
		return ResourceCache::has(M_GET_PACKEDSCENE_PATH(id));
	}
	_FORCE_INLINE_ void load() {
		ERR_FAIL_COND_MSG(packed_scenes.has(this), "PackedScene exist on Load!");
		String ppath = M_GET_PACKEDSCENE_PATH(id);
		if (ResourceCache::has(ppath)) {
			Ref<PackedScene> pscene = ResourceCache::get_ref(ppath);
