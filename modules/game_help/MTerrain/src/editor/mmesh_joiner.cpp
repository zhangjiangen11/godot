...
void MMeshJoiner::_join_meshes(const Vector<int> &data_ids, Array &mesh_arr, uint64_t flags) {
	...
	for (int id : data_ids) {
		...
		if ((flags & Flags::TANGENT) != 0) {
			m->append_tangents_to(tangents);
		}
		if ((flags & Flags::UV) != 0) {
			m->append_uv_to(uv);
		}
		if ((flags & Flags::UV2) != 0) {
			m->append_uv2_to(uv2);
		}
		if ((flags & Flags::COLOR) != 0) {
			m->append_colors_to(colors);
		}
	}
	...
	if ((flags & Flags::TANGENT) != 0) {
		mesh_arr[Mesh::ARRAY_TANGENT] = tangents;
	}
	if ((flags & Flags::UV) != 0) {
		mesh_arr[Mesh::ARRAY_TEX_UV] = uv;
	}
	if ((flags & Flags::UV2) != 0) {
		mesh_arr[Mesh::ARRAY_TEX_UV2] = uv2;
	}
	if ((flags & Flags::COLOR) != 0) {
	...
	for (int i = 0; i < meshes.size(); i++) {
		...
		for (int s = 0; s < surface_count; s++) {
			...
			if (s < surface_materials_path.size() && !surface_materials_path[s].is_empty()) {
				mdata.material = ResourceLoader::load(surface_materials_path[s]);
			}
			// push to data
			data.push_back(mdata);
		}
