		...
		for (int32_t y = 0; y < vert_num; y++) {
			for (int32_t x = 0; x < vert_num; x++) {
				...
				if (is_main && !(x == 0 || x == vert_num_minus_one || y == 0 || y == vert_num_minus_one)) {
					...
					seg.resize(24);
					seg.write[0] = index;
					seg.write[1] = rows[y][x - 1];
					seg.write[2] = rows[y - 1][x - 1]; //1
					seg.write[3] = index;
					seg.write[4] = rows[y - 1][x - 1];
					seg.write[5] = rows[y - 1][x]; //2
					seg.write[6] = index;
					seg.write[7] = rows[y - 1][x];
					seg.write[8] = rows[y - 1][x + 1]; //3
					seg.write[9] = index;
					seg.write[10] = rows[y - 1][x + 1];
					seg.write[11] = rows[y][x + 1]; //4
					seg.write[12] = index;
					seg.write[13] = rows[y][x + 1];
					seg.write[14] = rows[y + 1][x + 1]; //5
					seg.write[15] = index;
					seg.write[16] = rows[y + 1][x + 1];
					seg.write[17] = rows[y + 1][x]; //6
					seg.write[18] = index;
					seg.write[19] = rows[y + 1][x];
					seg.write[20] = rows[y + 1][x - 1]; //7
					seg.write[21] = index;
					seg.write[22] = rows[y + 1][x - 1];
					seg.write[23] = rows[y][x - 1]; //8
					indices.append_array(seg);
				}
				if (x == 1 && y != 0 && y != vert_num_minus_one) {
					if (fmod(y, 2) == 0) {
						indices.append(index);
						indices.append(rows[y][x - 1]);
						indices.append(rows[y - 1][x]);
						indices.append(index);
						indices.append(rows[y + 1][x]);
						indices.append(rows[y][x - 1]);
					} else if (el) {
						indices.append(index);
						indices.append(rows[y + 1][x - 1]);
						indices.append(rows[y - 1][x - 1]);
					} else {
						indices.append(index);
						indices.append(rows[y][x - 1]);
