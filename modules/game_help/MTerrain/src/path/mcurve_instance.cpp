...
void MCurveInstanceElement::_generate_transforms() {
	for (int i = 0; i < transform_count; i++) {
		...
		b.rotate(Vector3(0, 1, 0), VariantUtilityFunctions::deg_to_rad(rotation.y));
		b.rotate(Vector3(1, 0, 0), VariantUtilityFunctions::deg_to_rad(rotation.x));
		b.rotate(Vector3(0, 0, 1), VariantUtilityFunctions::deg_to_rad(rotation.z));
		// Rotation order YXZ
...
void MCurveInstanceOverride::set_flag(int64_t conn_id, int element_index, FLAG flag, bool value) {
	...
	if (!ov.has_any_override()) {
		VariantUtilityFunctions::_print("erase on set flag!");
		data.erase(conn_id);
	}
	emit_connection_changed(conn_id);
}
...
void MCurveInstance::_generate_connection(const MCurve::ConnUpdateInfo &update_info, bool immediate_update) {
	...
	// Getting instances
	Instances *instances;
	HashMap<int64_t, MCurveInstance::Instances>::Iterator it = curve_instance_instances.find(cid);
	...
	for (int instance_index = 0; instance_index < M_CURVE_CONNECTION_INSTANCE_COUNT; instance_index++) {
		...
		//////////////////////////////////////////////////////////////////////////////////////////////
		{
			...
			if (item_count == 0) {
				_remove_instance(cid, instance_index, false);
				VariantUtilityFunctions::_print("s100");
				continue;
			}
			// Adding meshes
			if (mesh.is_valid()) {
...
void MCurveInstance::_remove_instance(int64_t conn_id, int instance_index, bool rm_curve_instance_instances) {
	...
	if (instance_index != -1) {
		...
		if (instances[instance_index].body.is_valid()) {
			PS->free(instances[instance_index].body);
