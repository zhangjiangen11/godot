...
void FoliageCellGpuInstanceLoadBuffer::AddLoadInfo(FoliageCellAsset::CellData &info, MemoryPool::Block *dest_point, const Vector3 &mapOffset, const AABB &boxOS, const Vector2i &instanceRange) {
	...
	memcpy(tempLoadCellData.ptrw() + copy_index, info.instances.ptrw() + instanceRange.x, (instanceRange.y - instanceRange.x) * sizeof(FoliageCellAsset::InstanceData));
	...
	loadInfo->_cellOffset.y = FoliageGlobals::FOLIAGE_CELL_SIZE;
}
} //namespace Foliage
