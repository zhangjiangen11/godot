...
void FoliageEngine::add_prototype(FoliagePrototypeAsset *_proto) {
	...
	FoliageCellPos pos;
	pos.x = r.position.x / FoliageGlobals::FOLIAGE_PAGE_SIZE;
	pos.z = r.position.y / FoliageGlobals::FOLIAGE_PAGE_SIZE;
...
void FoliageEngine::update_foliage_asset_load(const Vector3 &camera_pos) {
	int map_index_x = camera_pos.x / (FoliageGlobals::FOLIAGE_CELL_SIZE * FoliageGlobals::FOLIAGE_PAGE_SIZE * map_page_count);
	int map_index_z = camera_pos.z / (FoliageGlobals::FOLIAGE_CELL_SIZE * FoliageGlobals::FOLIAGE_PAGE_SIZE * map_page_count);
