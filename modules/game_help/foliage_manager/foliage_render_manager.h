...
public:
	...
	void load_once_cell_box(PendingProtoBox *_cellBox) {
		...
		if (!renderBlock.has(key) && cell != nullptr) {
			...
			// 增加到加載列表
			foliageRenderBuffer.cellInstanceLoadBuffer.AddLoadInfo(*cell, item->block, _cellBox->cellWorldPos.worldPosition(), _cellBox->box, Vector2i(_cellBox->instanceStart, _cellBox->instanceStart + _cellBox->instanceCount));
	...
	void thread_procell_cull(int index) {
		...
		if ((_cellBox->loadTag == FoliageGlobals::LOAD_TAG_PRE_UNLOAD || _cellBox->loadTag == FoliageGlobals::LOAD_TAG_NONE) && _distanceToCameraSq < _cellBox->distance) {
