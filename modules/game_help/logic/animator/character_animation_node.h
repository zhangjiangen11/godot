...
public:
	...
	void set_human_config(const Ref<HumanBoneConfig> &p_human_config) { human_config = p_human_config; }
	Ref<HumanBoneConfig> get_human_config() { return human_config; }
...
public:
	...
	void set_blend_type(BlendType p_blend_type) {
		blend_type = (BlendType)p_blend_type;
	...
	void set_position_x(uint32_t p_index, float p_value) {
		blend_data.position_array[p_index].x = p_value;
		blend_data.is_init_precompute = false;
	}
	float get_position_x(uint32_t p_index) { return blend_data.position_array[p_index].x; }
	void set_position_y(uint32_t p_index, float p_value) {
		blend_data.position_array[p_index].y = p_value;
		blend_data.is_init_precompute = false;
	}
