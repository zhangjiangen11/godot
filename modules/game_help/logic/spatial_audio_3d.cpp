...
void Soundplayer::set_delay_ms(int v) {
	if (Math::abs(delay_ms - v) > 10 && (OS::get_singleton()->get_ticks_msec() - delay_updated_at) > 1000) {
		...
		if (state != fading_to_inactive) {
			set_audioeffect(audiobus_name, delay, Dictionary{
														  /* initializer lists are unsupported */ { "delay", delay_ms },
												  });
		}
	}
}
...
void Soundplayer::set_room_size(double v) {
	if (room_size != v) {
		room_size = v;
		if (state != fading_to_inactive) {
			set_audioeffect(audiobus_name, reverb, Dictionary{
														   /* initializer lists are unsupported */ { "room_size", room_size },
														   { "wetness", wetness },
												   });
		}
	}
...
void Soundplayer::set_proximity_bass(double v) {
	if (proximity_bass != v) {
		proximity_bass = v;
		if (state != fading_to_inactive) {
			set_audioeffect(audiobus_name, reverb_hipass, Dictionary{
																  /* initializer lists are unsupported */ { "hipass", proximity_bass },
														  });
		}
	}
}
...
void Soundplayer::set_lp_cutoff(int v) {
	if (Math::abs(lp_cutoff - v) > 20) {
		...
		set_audioeffect(audiobus_name, lowpass, Dictionary{
														/* initializer lists are unsupported */ { "lowpass", lp_cutoff },
														{ "fadetime", occlusion_fadetime },
												});
	}
}
