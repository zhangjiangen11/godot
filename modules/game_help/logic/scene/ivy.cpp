...
void Ivy::grow()
{
	...
	for (std::vector<IvyRoot>::iterator root = roots.begin(); root != roots.end(); ++root)
	{
		...
		if (root->nodes.back().floatingLength > maxFloatLength) root->alive = false;
		...
		//grow vectors: primary direction, random influence, and adhesion of scene objectss
			...
			//compute grow vector
			Vector3 growVector = ivySize * (primaryVector * primaryWeight +
		...
		//create next ivy node
			...
			if(tmpNode.length > maxLength) maxLength = tmpNode.length;
