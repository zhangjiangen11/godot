...
class TreeFactory {
	...
	using Parameter = ProceduralTreeParameter;
...
public:
	...
	/**
	 * Get all names of tree model.
	 */
	void GetAllTreeNames(Array<std::string> *tree_names) {
		//CHECK(tree_names);

		tree_names->clear();
		for (auto i : function_map_) {
			tree_names->push_back(i.first);
		}
	}
