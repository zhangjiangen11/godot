...
DebugShow::DebugShow() {
	Ref<StandardMaterial3D> _material = memnew(StandardMaterial3D);

	const Color collision_color(1.0, 1.0, 1.0, 1.0);

	_material->set_albedo(collision_color);
	_material->set_shading_mode(StandardMaterial3D::SHADING_MODE_UNSHADED);
	_material->set_transparency(StandardMaterial3D::TRANSPARENCY_ALPHA);
	_material->set_render_priority(StandardMaterial3D::RENDER_PRIORITY_MIN + 1);
	_material->set_cull_mode(StandardMaterial3D::CULL_BACK);
	_material->set_flag(StandardMaterial3D::FLAG_DISABLE_FOG, true);
	_material->set_flag(StandardMaterial3D::FLAG_ALBEDO_FROM_VERTEX_COLOR, true);
	_material->set_flag(StandardMaterial3D::FLAG_SRGB_VERTEX_COLOR, true);
