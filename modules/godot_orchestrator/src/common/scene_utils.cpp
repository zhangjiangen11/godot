...
{
    Ref<Texture2D> _get_class_or_script_icon(const String& p_class_name, const Ref<Script>& p_script,
                                             const String& p_fallback, bool p_fallback_script_to_theme)
    {
        ...
        {
            const String icon = OrchestratorScriptServer::get_global_class(p_class_name).icon_path;
            if (!icon.is_empty())
                return ResourceLoader::load(icon);

            return get_class_icon(OrchestratorScriptServer::get_native_class_name(p_class_name));
        }
    ...
    {
        return ResourceLoader::load(vformat(get_icon_path(p_name)));
    }
    ...
    Node* get_relative_scene_root(Node* p_node)
    {
        ...
        if (!node_scene_file.is_empty() && !node_owner_scene_file.is_empty() && node_scene_file != node_owner_scene_file)
            return p_node;
