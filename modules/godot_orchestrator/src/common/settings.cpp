...
bool OrchestratorSettings::has_setting(const String& key) const
{
    ...
    if (!result)
        print_line("Failed to find key ", path);
...
{
    return ProjectSettings::get_singleton()->get_setting("orchestrator/settings/action_favorites", PackedStringArray());
}
...
void OrchestratorSettings::add_action_favorite(const String& p_action_name)
{
    ...
    if (!ps->has_setting(key))
    {
        ...
        ps->set_custom_property_info(pi);
...
void OrchestratorSettings::_register_settings()
{
    ...
    _settings.emplace_back(FILE_SETTING("settings/dialogue/default_message_scene", "*.tscn,*.scn",
                                        "res://addons/orchestrator/scenes/dialogue_message.tscn"));
    ...
    _settings.emplace_back(
        COLOR_NO_ALPHA_SETTING("ui/node_colors/other_script_function_call", Color(0.027, 0.341, 0.504)));
