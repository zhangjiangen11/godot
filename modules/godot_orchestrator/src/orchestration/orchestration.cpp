...
void Orchestration::_fix_orphans()
{
    ...
    for (const KeyValue<int, Ref<OScriptNode>>& E : _nodes)
    {
        ...
        WARN_PRINT(
            vformat("Removed orphan node %d (%s) from script %s.", E.key, E.value->get_class(), get_self()->get_path()));
        _nodes.erase(E.key);
    }

    {
        ...
        for (const OScriptConnection C : removals)
        {
            ...
            WARN_PRINT(vformat("Removing orphan connection for " + C.to_string()
                               + ", either the source or target node no longer exists." + extra));
...
Ref<OScriptFunction> Orchestration::create_function(const MethodInfo& p_method, int p_node_id, bool p_user_defined)
{
    ...
    ERR_FAIL_COND_V_MSG(_functions.has(p_method.name), nullptr,
                        "A function already exists with the name: " + p_method.name);
    ERR_FAIL_COND_V_MSG(_variables.has(p_method.name), nullptr,
                        "A variable already exists with the name: " + p_method.name);
...
bool Orchestration::rename_function(const StringName& p_old_name, const StringName& p_new_name)
{
    ...
    ERR_FAIL_COND_V_MSG(!has_function(p_old_name), false,
                        "Cannot rename, no function found with old name: " + p_old_name);
    ERR_FAIL_COND_V_MSG(has_function(p_new_name), false,
                        "Cannot rename, a function already exists with new name: " + p_new_name);
    ERR_FAIL_COND_V_MSG(!String(p_new_name).is_valid_identifier(), false, "New function name is invalid: " + p_new_name);
    ERR_FAIL_COND_V_MSG(has_variable(p_new_name), false,
                        "Cannot rename function, a variable with name already exists: " + p_new_name);
    ERR_FAIL_COND_V_MSG(has_custom_signal(p_new_name), false,
                        "Cannot rename function, a signal with the name already exists: " + p_new_name);
...
Ref<OScriptVariable> Orchestration::create_variable(const StringName& p_name, Variant::Type p_type)
{
    ...
    ERR_FAIL_COND_V_MSG(!String(p_name).is_valid_identifier(), nullptr,
                        "Cannot create variable, invalid name: " + p_name);
...
bool Orchestration::rename_variable(const StringName& p_old_name, const StringName& p_new_name)
{
    ...
    ERR_FAIL_COND_V_MSG(!has_variable(p_old_name), false,
                        "Cannot rename, no variable exists with the old name: " + p_old_name);
    ERR_FAIL_COND_V_MSG(has_variable(p_new_name), false,
                        "Cannot rename, a variable already exists with the new name: " + p_new_name);
    ERR_FAIL_COND_V_MSG(!String(p_new_name).is_valid_identifier(), false,
                        "Cannot rename, variable name is not valid: " + p_new_name);
...
bool Orchestration::rename_custom_user_signal(const StringName& p_old_name, const StringName& p_new_name)
{
    ...
    ERR_FAIL_COND_V_MSG(!has_custom_signal(p_old_name), false,
                        "No custom signal exists with the old name: " + p_old_name);
    ERR_FAIL_COND_V_MSG(has_custom_signal(p_new_name), false,
                        "A custom signal already exists with the new name: " + p_new_name);
    ERR_FAIL_COND_V_MSG(!String(p_new_name).is_valid_identifier(), false,
                        "The custom signal name is invalid: " + p_new_name);
