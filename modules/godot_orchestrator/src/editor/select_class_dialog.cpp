...
Vector<Ref<OrchestratorEditorSearchDialog::SearchItem>> OrchestratorSelectClassSearchDialog::_get_class_hierarchy_search_items(
    const String& p_class, HashMap<String, Ref<SearchItem>>& r_cache, const Ref<SearchItem>& p_root)
{
    ...
    for (; class_index < hierarchy.size(); class_index++)
    {
        ...
        if (OrchestratorScriptServer::is_global_class(class_name))
            item->script_filename = OrchestratorScriptServer::get_global_class(class_name).path.get_file();
...
void OrchestratorSelectClassSearchDialog::popup_create(bool p_dont_clear, bool p_replace_mode,
