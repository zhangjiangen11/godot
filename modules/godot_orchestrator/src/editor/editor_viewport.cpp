...
void OrchestratorEditorViewport::_resolve_node_set_connections(const Vector<Ref<OScriptNode>>& p_nodes,
                                                               NodeSetConnections& r_connections)
{
...
OrchestratorGraphEdit* OrchestratorEditorViewport::_get_or_create_tab(const StringName& p_name, bool p_focus,
                                                                      bool p_create)
{
...
bool OrchestratorEditorViewport::build(bool p_show_success)
{
    ...
    if (!failures.is_empty())
    {
        ...
        for (const BuildLog::Failure& failure : failures)
        {
            ...
            {
                String pin_name = StringUtils::default_if_empty(failure.pin->get_label(),
                                                                failure.pin->get_pin_name().capitalize());
            ...
            const String text = vformat("Node #[url={\"goto_node\":\"%d\",\"script\":\"%s\"}]%d - %s[/url]\n\t%s",
                                        failure.node->get_id(), failure.node->get_orchestration()->get_self()->get_path(),
                                        failure.node->get_id(), failure.node->get_node_title(), message);
...
void OrchestratorEditorViewport::_notification(int p_what)
{
    if (p_what == NOTIFICATION_READY)
    {
        ...
        _tabs->get_tab_bar()->connect("tab_close_pressed",
                                      callable_mp(this, &OrchestratorEditorViewport::_close_tab_requested));
...
OrchestratorEditorViewport::OrchestratorEditorViewport(const Ref<Resource>& p_resource)
