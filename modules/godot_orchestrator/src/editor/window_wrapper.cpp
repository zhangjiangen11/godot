...
void OrchestratorWindowWrapper::_notification(int p_what)
{
    ...
    switch (p_what)
    {
        ...
        case NOTIFICATION_ENTER_TREE:
            _window->connect("close_requested",
                             callable_mp(this, &OrchestratorWindowWrapper::set_window_enabled).bind(false));
            break;
...
void OrchestratorWindowWrapper::restore_window_from_saved_position(const Rect2 p_window_rect, int p_screen,
...
void OrchestratorScreenSelect::_notification(int p_what)
{
    switch (p_what)
    {
        case NOTIFICATION_READY:
            _popup->connect(
                "popup_hide",
                callable_mp(static_cast<BaseButton*>(this), &OrchestratorScreenSelect::set_pressed).bind(false));
        ...
        case NOTIFICATION_THEME_CHANGED:
        {
            ...
            _popup_background->add_theme_stylebox_override("panel",
                                                           get_theme_stylebox("PanelForeground", "EditorStyles"));
...
void OrchestratorScreenSelect::_build_advanced_menu()
{
    ...
    for (int i = 0; i < DisplayServer::get_singleton()->get_screen_count(); i++)
    {
        ...
        button->connect(
            "pressed", callable_mp(static_cast<BaseButton*>(this), &OrchestratorScreenSelect::set_pressed).bind(false));
