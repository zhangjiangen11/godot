...
void OrchestratorBuildOutputPanel::reset()
{
    ...
    _rtl->clear();
    _rtl->append_text(
        vformat("%s - (c) 2023-present Crater Crash Studios, LLC and its contributors.\n\n", VERSION_FULL_NAME));
}
...
void OrchestratorBuildOutputPanel::_notification(int p_what)
{
    ...
    else if (p_what == NOTIFICATION_READY)
    {
        ...
        _clear_button->connect("pressed", callable_mp_lambda(this, [this] {
                                   reset();
                               }));
    }
}
...
{
    ADD_SIGNAL(MethodInfo("meta_clicked",
