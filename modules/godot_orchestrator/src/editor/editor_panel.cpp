...
void OrchestratorEditorPanel::_update_scene_tab_signals(bool p_connect)
{
    ...
    if (p_connect)
    {
        ...
        if (debugger)
        {
            ...
            OCONNECT(debugger, "breakpoints_cleared_in_tree",
                     callable_mp(this, &OrchestratorEditorPanel::_clear_all_breakpoints));
    ...
    if (debugger)
    {
        ...
        ODISCONNECT(debugger, "breakpoints_cleared_in_tree",
                    callable_mp(this, &OrchestratorEditorPanel::_clear_all_breakpoints));
...
{
    const Ref<Resource> resource = ResourceLoader::load(p_file_name);
...
void OrchestratorEditorPanel::_queue_close_scripts()
{
    while (!_files_context.close_queue.is_empty())
    {
        ...
        {
            file.viewport->connect("tree_exited", callable_mp(this, &OrchestratorEditorPanel::_queue_close_scripts),
                                   CONNECT_ONE_SHOT);
            _ask_close_current_unsaved_editor();
            break;
        }
...
{
    ERR_FAIL_COND_MSG(!p_script.is_valid() || !p_script->get_class().match(OScript::get_class_static()),
                      "The script is not an orchestration.");
...
void OrchestratorEditorPanel::_add_script_function(Object* p_object, const String& p_function_name,
...
void OrchestratorEditorPanel::set_window_layout(const Ref<ConfigFile>& p_configuration)
{
    ...
    for (const String& file_name : open_files)
        edit_resource(ResourceLoader::load(file_name));
    ...
    switch (p_what)
    {
        case NOTIFICATION_READY:
        {
            ...
            if (Node* editor_node = get_tree()->get_root()->get_child(0))
                editor_node->connect("script_add_function_request",
                                     callable_mp(this, &OrchestratorEditorPanel::_add_script_function));
            ...
            _recent_history->connect("index_pressed",
                                     callable_mp(this, &OrchestratorEditorPanel::_recent_history_selected));
            ...
            _file_menu->get_popup()->add_item("Save All", FILE_SAVE_ALL,
                                              OACCEL_KEY(KEY_MASK_SHIFT | KEY_MASK_ALT, KEY_S));
            _file_menu->get_popup()->add_separator();
            ...
            _file_menu->get_popup()->add_separator();
            _file_menu->get_popup()->add_item("Toggle Orchestration List", FILE_TOGGLE_LEFT_PANEL,
                                              OACCEL_KEY(KEY_MASK_CTRL, KEY_BACKSLASH));
            _file_menu->get_popup()->add_item("Toggle Component Panel", FILE_TOGGLE_RIGHT_PANEL,
                                              OACCEL_KEY(KEY_MASK_CTRL, KEY_SLASH));
            _file_menu->get_popup()->connect("id_pressed",
                                             callable_mp(this, &OrchestratorEditorPanel::_handle_menu_option));
            _file_menu->get_popup()->connect("about_to_popup",
                                             callable_mp(this, &OrchestratorEditorPanel::_prepare_file_menu));
            ...
            _goto_menu->get_popup()->connect("id_pressed",
                                             callable_mp(this, &OrchestratorEditorPanel::_handle_menu_option));
            _goto_menu->get_popup()->connect("about_to_popup",
                                             callable_mp(this, &OrchestratorEditorPanel::_prepare_goto_menu));
            ...
            _help_menu->get_popup()->add_icon_item(SceneUtils::get_editor_icon("ExternalLink"), "Online Documentation",
                                                   HELP_ONLINE_DOCUMENTATION);
            _help_menu->get_popup()->add_icon_item(SceneUtils::get_editor_icon("ExternalLink"), "Community",
                                                   HELP_COMMUNITY);
            _help_menu->get_popup()->add_separator();
            _help_menu->get_popup()->add_icon_item(SceneUtils::get_editor_icon("ExternalLink"), "Report a Bug",
                                                   HELP_GITHUB_ISSUES);
            _help_menu->get_popup()->add_icon_item(SceneUtils::get_editor_icon("ExternalLink"), "Suggest a Feature",
                                                   HELP_GITHUB_FEATURE);
            _help_menu->get_popup()->add_separator();
            _help_menu->get_popup()->add_item("About " VERSION_NAME, HELP_ABOUT);
            _help_menu->get_popup()->add_icon_item(SceneUtils::get_editor_icon("Heart"), "Support " VERSION_NAME,
                                                   HELP_SUPPORT);
            _help_menu->get_popup()->connect("id_pressed",
                                             callable_mp(this, &OrchestratorEditorPanel::_handle_menu_option));
            ...
            open_docs->call_deferred(
                "connect", "pressed",
                callable_mp(this, &OrchestratorEditorPanel::_handle_menu_option).bind(HELP_ONLINE_DOCUMENTATION));
            ...
            if (_window_wrapper->is_window_available())
            {
                ...
                _screen_select->connect(
                    "request_open_in_screen",
                    callable_mp(_window_wrapper, &OrchestratorWindowWrapper::enable_window_on_screen).bind(true));
                right_menu->add_child(_screen_select);
                _window_wrapper->connect("window_visibility_changed",
                                         callable_mp(this, &OrchestratorEditorPanel::_window_changed));
            }
            ...
            _file_list->connect("item_clicked",
                                callable_mp(this, &OrchestratorEditorPanel::_show_file_list_context_menu));
            ...
            _file_list_context_menu->add_separator();
            _file_list_context_menu->add_item("Toggle Orchestration List", FILE_TOGGLE_LEFT_PANEL,
                                              OACCEL_KEY(KEY_MASK_CTRL, KEY_BACKSLASH));
            _file_list_context_menu->connect("id_pressed",
                                             callable_mp(this, &OrchestratorEditorPanel::_handle_menu_option));
            ...
            _getting_started->connect("create_requested",
                                      callable_mp(this, &OrchestratorEditorPanel::_handle_menu_option).bind(FILE_NEW));
            _getting_started->connect("open_requested",
                                      callable_mp(this, &OrchestratorEditorPanel::_handle_menu_option).bind(FILE_OPEN));
            _getting_started->connect(
            ...
            _close_confirm->connect("custom_action",
                                    callable_mp(this, &OrchestratorEditorPanel::_close_tab_discard_changes));
            ...
            _script_create_dialog->connect("script_created",
                                           callable_mp(this, &OrchestratorEditorPanel::_script_file_created));
            add_child(_script_create_dialog);

            OrchestratorPlugin::get_singleton()->get_build_panel()->connect(
                "meta_clicked", callable_mp(this, &OrchestratorEditorPanel::_build_log_meta_clicked));
