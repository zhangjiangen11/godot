...
OrchestratorVersion OrchestratorVersion::parse(const String& p_tag_version)
{
    ...
    const PackedStringArray parts = (p_tag_version.begins_with("v") ? p_tag_version.substr(1).split(".") :
                                                                      p_tag_version.split("."));
...
{
    return major == p_other.major && minor == p_other.minor && patch == p_other.patch
           && build.name == p_other.build.name && build.version == p_other.build.version;
}
...
void OrchestratorUpdaterReleaseNotesDialog::_notification(int p_what)
{
    ...
    {
        connect("canceled", callable_mp_lambda(this, [this] {
                    queue_free();
                }));
        connect("confirmed", callable_mp_lambda(this, [this] {
                    queue_free();
                }));
    }
}
...
void OrchestratorUpdaterVersionPicker::_check_godot_compatibility()
{
    ...
    if (selected)
    {
        if (!selected->get_meta("compatible", false))
        {
            ...
            add_child(notify);
            notify->connect("canceled", callable_mp_lambda(this, [notify] {
                                notify->queue_free();
                            }));
            notify->connect("confirmed", callable_mp_lambda(this, [notify] {
                                notify->queue_free();
                            }));
...
void OrchestratorUpdaterVersionPicker::_download_completed(int p_status, int p_code, const PackedStringArray& p_headers,
...
void OrchestratorUpdaterVersionPicker::_update_tree(bool p_stable_only)
{
    ...
    for (const ReleaseItem& release_item : _releases)
    {
        ...
        {
            item->add_button(0, SceneUtils::get_editor_icon("KeyXScale"), -1, true,
                             "Your Godot version is not compatible");
...
void OrchestratorUpdaterVersionPicker::add_release(const OrchestratorRelease& p_release,
                                                   const String& p_godot_compatibility, const String& p_blog_url)
{
    ReleaseItem item;
...
void OrchestratorUpdaterVersionPicker::_notification(int p_what)
{
    ...
    {
        _download->connect("request_completed",
                           callable_mp(this, &OrchestratorUpdaterVersionPicker::_download_completed));
        _release_filter->connect("item_selected", callable_mp(this, &OrchestratorUpdaterVersionPicker::_filter_changed));
        _notify_any_release->connect("pressed",
                                     callable_mp(this, &OrchestratorUpdaterVersionPicker::_update_notify_settings));
        _tree->connect("item_activated",
                       callable_mp(this, &OrchestratorUpdaterVersionPicker::_check_godot_compatibility));
        _tree->connect("item_selected",
                       callable_mp(this, &OrchestratorUpdaterVersionPicker::_set_button_enable_state).bind(true));
...
#endif
                _status->set_text(
                    vformat("Downloading... (%s)", String::humanize_size(_download->get_downloaded_bytes())));
            }
        }
    }
}
...
OrchestratorUpdaterVersionPicker::OrchestratorUpdaterVersionPicker()
{
    ...
    _godot_version = OrchestratorVersion::parse(
        vformat("v%d.%d.%d", gd_version.major, gd_version.minor, gd_version.patch));
...
Error OrchestratorUpdaterButton::_send_http_request(const String& p_url, const String& p_filename,
                                                    const Callable& p_callback)
{
    ...
    request->connect("request_completed",
                     callable_mp_lambda(this,
                                        [=](int p_result, int p_code, const PackedStringArray& p_headers,
                                            const PackedByteArray& p_data) {
...
void OrchestratorUpdaterButton::_process_release_manifests()
{
    ...
    const String cache_dir = EditorInterface::get_singleton()->get_editor_paths()->get_cache_dir();
    const PackedByteArray bytes = FileAccess::get_file_as_bytes(
...
void OrchestratorUpdaterButton::_check_for_updates()
{
    ...
    _send_http_request(VERSION_RELEASES_URL, cache_dir.path_join("tmp_orchestrator_releases.json"),
                       callable_mp(this, &OrchestratorUpdaterButton::_process_releases));

    _send_http_request(VERSION_MANIFESTS_URL, cache_dir.path_join("tmp_orchestrator_release_manifests.json"),
...
OrchestratorUpdaterButton::OrchestratorUpdaterButton()
{
    ...
    _plugin_version = OrchestratorVersion::parse(
        vformat("v%d.%d.%d.%s", VERSION_MAJOR, VERSION_MINOR, VERSION_MAINTENANCE, VERSION_STATUS));
}