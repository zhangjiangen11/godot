...
bool OrchestratorScriptFunctionsComponentPanel::_populate_context_menu(TreeItem* p_item)
{
    ...
    if (p_item->has_meta("__slot") && p_item->get_meta("__slot"))
    {
        ...
        _context_menu->set_item_tooltip(_context_menu->get_item_index(CM_DISCONNECT_SLOT),
...
void OrchestratorScriptFunctionsComponentPanel::_notification(int p_what)
{
    ...
    if (p_what == NOTIFICATION_READY)
    {
        ...
        _slot_update_timer->connect("timeout",
                                    callable_mp(this, &OrchestratorScriptFunctionsComponentPanel::_update_slots));
        ...
        _override_button->connect("pressed", callable_mp_lambda(this, [=, this] {
...
void OrchestratorScriptFunctionsComponentPanel::_bind_methods()
{
    ...
    ADD_SIGNAL(MethodInfo("graph_renamed", PropertyInfo(Variant::STRING, "old_name"),
                          PropertyInfo(Variant::STRING, "new_name")));
    ADD_SIGNAL(MethodInfo("focus_node_requested", PropertyInfo(Variant::STRING, "graph_name"),
                          PropertyInfo(Variant::INT, "node_id")));
    ADD_SIGNAL(MethodInfo("override_function_requested"));
}

OrchestratorScriptFunctionsComponentPanel::OrchestratorScriptFunctionsComponentPanel(Orchestration* p_orchestration,
                                                                                     Callable p_new_function_callback)
    : OrchestratorScriptComponentPanel("Functions", p_orchestration)
    , _new_function_callback(p_new_function_callback)
{
}
