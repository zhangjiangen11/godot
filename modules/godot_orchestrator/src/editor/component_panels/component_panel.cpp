...
void OrchestratorScriptComponentPanel::_tree_item_button_clicked(TreeItem* p_item, int p_column, int p_id,
                                                                 int p_mouse_button)
{
...
TreeItem* OrchestratorScriptComponentPanel::_create_item(TreeItem* p_parent, const String& p_text,
                                                         const String& p_item_name, const String& p_icon_name)
{
                ...
                if (p_edit)
                {
                    ...
                    if (timer.is_valid())
                        timer->connect("timeout",
                                       callable_mp(this, &OrchestratorScriptComponentPanel::_edit_selected_tree_item));
                }
...
void OrchestratorScriptComponentPanel::_notification(int p_what)
{
    if (p_what == NOTIFICATION_READY)
    {
        ...
        _tree->connect("item_selected", callable_mp_lambda(this, [&] {
                           _handle_item_selected();
                       }));
        _tree->connect("item_mouse_selected",
                       callable_mp(this, &OrchestratorScriptComponentPanel::_tree_item_mouse_selected));
        _tree->connect("item_collapsed", callable_mp_lambda(this, [&]([[maybe_unused]] TreeItem* i) {
        ...
        _context_menu->connect("id_pressed", callable_mp_lambda(this, [&](int id) {
                                   _handle_context_menu(id);
                               }));
        _confirm->connect("confirmed", callable_mp(this, &OrchestratorScriptComponentPanel::_remove_confirmed));

        _tree->set_drag_forwarding(callable_mp(this, &OrchestratorScriptComponentPanel::_tree_drag_data), Callable(),
                                   Callable());
    }
