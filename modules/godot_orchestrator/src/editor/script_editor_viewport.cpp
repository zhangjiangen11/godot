...
void OrchestratorScriptEditorViewport::_graph_opened(OrchestratorGraphEdit* p_graph)
{
    ...
    p_graph->connect("collapse_selected_to_function",
                     callable_mp(this, &OrchestratorScriptEditorViewport::_collapse_selected_to_function).bind(p_graph));
...
void OrchestratorScriptEditorViewport::_collapse_selected_to_function(OrchestratorGraphEdit* p_graph)
{
    ...
    ERR_FAIL_COND_EDMSG(connections.input_executions > 1,
                        "Cannot collapse to function with more than one external input execution wire.");
    ERR_FAIL_COND_EDMSG(connections.output_executions > 1,
                        "Cannot collapse to function with more than one external output execution wire.");
...
void OrchestratorScriptEditorViewport::_expand_node(int p_node_id, OrchestratorGraphEdit* p_graph)
{
    ...
    for (const Ref<OScriptNode>& node : selected)
    {
        const Ref<OScriptNode> duplicate = p_graph->get_owning_graph()->duplicate_node(node->get_id(), pos_delta, true);
...
void OrchestratorScriptEditorViewport::add_script_function(Object* p_object, const String& p_function_name,
...
void OrchestratorScriptEditorViewport::_notification(int p_what)
{
    ...
    if (p_what == NOTIFICATION_READY)
    {
        ...
        _functions->connect("override_function_requested",
                            callable_mp(this, &OrchestratorScriptEditorViewport::_override_godot_function));
