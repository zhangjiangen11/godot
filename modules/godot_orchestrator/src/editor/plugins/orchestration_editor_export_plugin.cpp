...
void OrchestratorEditorExportPlugin::_export_begin(const PackedStringArray& p_features, bool p_is_debug,
                                                   const String& p_path, uint32_t p_flags)
{
}

void OrchestratorEditorExportPlugin::_export_file(const String& p_path, const String& p_type,
                                                  const PackedStringArray& p_features)
{
    ...
    //
    // const String save_path = vformat("res://.godot/exported/orchestrator/export-%s-%s.os", p_path.md5_text(),
    // p_path.get_file().get_basename()); add_file(save_path, bytes, true);
}
