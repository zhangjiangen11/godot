...
void OrchestratorPlugin::_notification(int p_what)
{
    if (p_what == NOTIFICATION_ENTER_TREE)
    {
        ...
        _window_wrapper->connect("window_visibility_changed",
                                 callable_mp(this, &OrchestratorPlugin::_on_window_visibility_changed));
...
{
    return ResourceLoader::load(OScriptLanguage::ICON);
}
...
{
    return ResourceLoader::load("res://addons/orchestrator/icons/Orchestrator_Logo.svg");
}

Ref<ConfigFile> OrchestratorPlugin::get_metadata()
{
    const String file = get_editor_interface()->get_editor_paths()->get_project_settings_dir().path_join(
        "orchestrator_metadata.cfg");
...
void OrchestratorPlugin::request_editor_restart()
{
    ...
    request->connect("confirmed", callable_mp_lambda(this, [] {
