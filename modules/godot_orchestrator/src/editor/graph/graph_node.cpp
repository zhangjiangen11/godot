...
int OrchestratorGraphNode::get_inputs_with_opacity(float p_opacity)
{
    ...
    for (int i = 0; i < get_input_port_count(); i++)
    {
        ...
        {
            Color color = get_input_port_color(i);
            if (VariantUtilityFunctions::is_equal_approx(color.a, p_opacity))
                count++;
        }
    }
    return count;
    ...
    if (titlebar->get_child_count() > 0)
    {
        ...
        if (!_node->get_icon().is_empty())
        {
            if (_node->get_icon().begins_with("res://"))
                icon_texture = ResourceLoader::load(_node->get_icon());
            else
...
void OrchestratorGraphNode::_show_context_menu(const Vector2& p_position)
{
    ...
    for (const Ref<OScriptAction>& action : _context_actions)
    {
        if (action->get_icon().is_empty())
            ...
            _context_menu->add_icon_item(SceneUtils::get_editor_icon(action->get_icon()), action->get_text(),
                                         node_action_id);
    ...
    _context_menu->add_icon_item(SceneUtils::get_editor_icon("ActionCut"), "Cut", CM_CUT,
                                 OACCEL_KEY(KEY_MASK_CTRL, KEY_X));
    _context_menu->add_icon_item(SceneUtils::get_editor_icon("ActionCopy"), "Copy", CM_COPY,
                                 OACCEL_KEY(KEY_MASK_CTRL, KEY_C));
    _context_menu->add_icon_item(SceneUtils::get_editor_icon("Duplicate"), "Duplicate", CM_DUPLICATE,
                                 OACCEL_KEY(KEY_MASK_CTRL, KEY_D));
    ...
    alignment_menu->add_icon_item(SceneUtils::get_editor_icon("ControlAlignHCenterWide"), "Align Middle",
                                  CM_ALIGN_MIDDLE);
    ...
    alignment_menu->add_icon_item(SceneUtils::get_editor_icon("ControlAlignVCenterWide"), "Align Center",
                                  CM_ALIGN_CENTER);
    ...
    if (multi_selections)
        _context_menu->set_item_tooltip(_context_menu->get_item_index(CM_VIEW_DOCUMENTATION),
                                        "Select a single node to view documentation.");
