...
public:
    ...
    void for_each_graph_element(const std::function<void(GraphElement*)>& p_func, bool p_nodes = true,
                                bool p_knots = true);
    ...
    template <typename T>
    void spawn_node(const OScriptNodeInitContext& p_context, const Vector2& p_position = Vector2(),
    ...
    /// @param p_callback the callback to call when the node is spawned successfully.
    void spawn_node(const StringName& p_type, const OScriptNodeInitContext& p_context,
                    const Vector2& p_position = Vector2(), const Callable& p_callback = Callable());
...
private:
    ...
    bool _get_connection_for_points(const Vector2& p_from_position, const Vector2& p_to_position,
    ...
    template <typename T>
    T* _get_by_name(const StringName& p_name)
    {
        return Object::cast_to<T>(get_node_or_null(NodePath(p_name)));
    }
    ...
    void _queue_autowire(const Ref<OScriptNodePin>& p_source, const Ref<OScriptNode>& p_spawned,
                         const Callable& p_callback = Callable());
