...
void OrchestratorGraphNodeDefault::_on_row_ready(int p_row_index)
{
    if (_pin_rows.has(p_row_index))
    {
        ...
        set_slot(row.index, row.left && row.left->is_connectable() && !row.left->is_hidden(),
                 row.left ? row.left->get_slot_type() : 0, row.left ? row.left->get_color() : Color(0, 0, 0, 1),
                 row.right && row.right->is_connectable() && !row.right->is_hidden(),
                 row.right ? row.right->get_slot_type() : 0, row.right ? row.right->get_color() : Color(0, 0, 0, 1),
