...
void OrchestratorDefaultGraphActionRegistrar::_register_node(const StringName& p_class_name,
                                                             const StringName& p_category, const Dictionary& p_data)
{
    ...
    Ref<OrchestratorGraphActionHandler> handler(
        memnew(OrchestratorGraphNodeSpawnerScriptNode(p_class_name, p_data, node)));
...
OrchestratorGraphActionSpec OrchestratorDefaultGraphActionRegistrar::_get_method_spec(
    const MethodInfo& p_method, const String& p_base_type, const String& p_class_name)
{
    OrchestratorGraphActionSpec spec;
...
OrchestratorGraphActionSpec OrchestratorDefaultGraphActionRegistrar::_get_signal_spec(
    const StringName& p_signal_name, const String& p_base_type, const String& p_class_name)
{
    OrchestratorGraphActionSpec spec;
    if (p_class_name.is_empty())
...
void OrchestratorDefaultGraphActionRegistrar::_register_category(const String& p_category, const String& p_display_name,
                                                                 const String& p_icon)
{
    OrchestratorGraphActionSpec spec;
    spec.category = p_category;
...
void OrchestratorDefaultGraphActionRegistrar::_register_orchestration_nodes()
{
    ...
    const Dictionary array_data = DictionaryUtils::of(
        { { "collection_type", Variant::ARRAY }, { "index_type", Variant::NIL } });
    ...
    for (const String& class_name : ClassDB::get_class_list())
    {
        for (const String& function_name : ExtensionDB::get_static_function_names(class_name))
        {
            ...
            _register_node<OScriptNodeCallStaticFunction>(
                category, DictionaryUtils::of({ { "class_name", class_name }, { "method_name", function_name } }));
            ;
        }
    }

    // Static script function calls
    for (const String& global_class : OrchestratorScriptServer::get_global_class_list())
    {
        const TypedArray<Dictionary> methods
            = OrchestratorScriptServer::get_global_class(global_class).get_static_method_list();
        for (int i = 0; i < methods.size(); i++)
        {
            ...
            _register_node<OScriptNodeCallStaticFunction>(
                category, DictionaryUtils::of({ { "class_name", global_class }, { "method_name", method.name } }));
        }
    }
    ...
    for (const String& builtin_type_name : ExtensionDB::get_builtin_type_names())
    {
        ...
        if (type.index_returning_type != Variant::NIL && type.type >= Variant::ARRAY)
        {
            ...
            const Dictionary data = DictionaryUtils::of(
                { { "collection_type", type.type }, { "index_type", type.index_returning_type } });
...
void OrchestratorDefaultGraphActionRegistrar::_register_methods(const String& p_class_name,
                                                                const TypedArray<Dictionary>& p_methods)
{
...
void OrchestratorDefaultGraphActionRegistrar::_register_properties(const String& p_class_name,
                                                                   const TypedArray<Dictionary>& p_properties)
{
    OrchestratorScriptServer::GlobalClass global_class;
    if (OrchestratorScriptServer::is_global_class(p_class_name))
        global_class = OrchestratorScriptServer::get_global_class(p_class_name);
    ...
    for (int i = 0; i < p_properties.size(); i++)
    {
        ...
        {
            if (OrchestratorScriptServer::is_global_class(global_class.base_type))
            {
                if (OrchestratorScriptServer::get_global_class(global_class.base_type).has_property(pi.name))
                    continue;
            }
        }
        ...
        if (!has_getter)
        {
            ...
            Ref<OrchestratorGraphActionHandler> getter_handler(
                memnew(OrchestratorGraphNodeSpawnerPropertyGet(pi, Array::make(p_class_name))));
        ...
        if (!has_setter)
        {
            ...
            Ref<OrchestratorGraphActionHandler> setter_handler(
                memnew(OrchestratorGraphNodeSpawnerPropertySet(pi, Array::make(p_class_name))));
            _context->list->push_back(memnew(OrchestratorGraphActionMenuItem(setter_spec, setter_handler)));
        }
    }
}

void OrchestratorDefaultGraphActionRegistrar::_register_signals(const String& p_class_name,
                                                                const TypedArray<Dictionary>& p_signals)
{
    ...
    for (int i = 0; i < p_signals.size(); i++)
    {
        ...
        if (!global_class.name.is_empty() && OrchestratorScriptServer::is_global_class(global_class.base_type))
            if (OrchestratorScriptServer::get_global_class(global_class.base_type).has_signal(si.name))
                continue;
        ...
        Ref<OrchestratorGraphActionHandler> handler(
            memnew(OrchestratorGraphNodeSpawnerEmitMemberSignal(mi, p_class_name)));
        _context->list->push_back(memnew(OrchestratorGraphActionMenuItem(spec, handler)));
    }
}
...
void OrchestratorDefaultGraphActionRegistrar::_register_orchestration_signals()
{
    ...
    if (OrchestratorGraphEdit* graph = _context->graph)
    {
        ...
        for (const Ref<OScriptSignal>& signal : orchestration->get_custom_signals())
        {
            ...
            const OrchestratorGraphActionSpec spec = _get_signal_spec(signal->get_signal_name(),
                                                                      orchestration->get_base_type());

            Ref<OrchestratorGraphActionHandler> handler(
                memnew(OrchestratorGraphNodeSpawnerEmitSignal(signal->get_method_info())));
            _context->list->push_back(memnew(OrchestratorGraphActionMenuItem(spec, handler)));
        }
    }
}
...
void OrchestratorDefaultGraphActionRegistrar::register_actions(const OrchestratorGraphActionRegistrarContext& p_context)
{
    ...
    if (p_context.graph)
    {
        if (p_context.filter->has_target_object())
        {
            ...
            String global_name;
            if (script.is_valid())
                global_name = OrchestratorScriptServer::get_global_name(script);
            ...
            if (!global_name.is_empty())
            {
                ...
                const PackedStringArray script_class_names = OrchestratorScriptServer::get_class_hierarchy(global_name);
                for (const String& class_name : script_class_names)
                {
                    const OrchestratorScriptServer::GlobalClass global_class
                        = OrchestratorScriptServer::get_global_class(class_name);
        ...
        else if (!p_context.filter->target_classes.is_empty())
        {
            ...
            for (const StringName& target_class_name : p_context.filter->target_classes)
            {
                ...
                if (OrchestratorScriptServer::is_global_class(target_class_name))
                    class_names = OrchestratorScriptServer::get_class_hierarchy(target_class_name, true);
                else
                ...
                for (const String& class_name : class_names)
                {
                    if (!classes_added.has(class_name))
                    {
                        ...
                        if (OrchestratorScriptServer::is_global_class(class_name))
                        {
                            const OrchestratorScriptServer::GlobalClass global_class
                                = OrchestratorScriptServer::get_global_class(class_name);
                            _register_methods(class_name, global_class.get_method_list());
