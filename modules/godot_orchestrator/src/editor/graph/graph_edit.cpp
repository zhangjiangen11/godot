...
void OrchestratorGraphEdit::_notification(int p_what)
{
    if (p_what == NOTIFICATION_READY)
    {
        ...
        _script_graph->connect("connection_knots_removed",
                               callable_mp(this, &OrchestratorGraphEdit::_remove_connection_knots));
...
void OrchestratorGraphEdit::for_each_graph_element(const std::function<void(GraphElement*)>& p_func, bool p_nodes,
                                                   bool p_knots)
{
...
void OrchestratorGraphEdit::spawn_node(const StringName& p_type, const OScriptNodeInitContext& p_context,
                                       const Vector2& p_position, const Callable& p_callback)
{
...
void OrchestratorGraphEdit::_gui_input(const Ref<InputEvent>& p_event)
{
    ...
    if (me.is_valid() && !_is_position_valid_for_knot(me->get_position()))
    {
        ...
        if (mm.is_valid())
        {
            ...
            {
                _show_drag_hint(
                    "Use Ctrl + left click to add a knot to the connection.\n"
    ...
    if (mm.is_valid() && _box_selection)
    {
        ...
        const Rect2 select_rect = Rect2(_box_selection_from.min(selection_to),
                                        (_box_selection_from - selection_to).abs());
...
void OrchestratorGraphEdit::_drop_data(const Vector2& p_position, const Variant& p_data)
{
    ...
    else if (type == "variable")
    {
        ...
        if (Input::get_singleton()->is_key_pressed(KEY_CTRL))
        {
            if (!is_constant)
            {
                Ref<OrchestratorGraphNodeSpawnerVariableSet> setter(
        ...
        {
            Ref<OrchestratorGraphNodeSpawnerVariableGet> getter(
        ...
        else
        {
            ...
            _context_menu->set_item_metadata(_context_menu->get_item_index(CM_VARIABLE_GET),
                                             memnew(OrchestratorGraphNodeSpawnerVariableGet(variable_name)));

            if (is_validated)
            {
                _context_menu->add_item("Get " + variable_name + " with validation", CM_VARIABLE_GET_VALIDATED);
                _context_menu->set_item_metadata(_context_menu->get_item_index(CM_VARIABLE_GET_VALIDATED),
                                                 memnew(OrchestratorGraphNodeSpawnerVariableGet(variable_name, true)));
            }

            if (!is_constant)
            {
                _context_menu->add_item("Set " + variable_name, CM_VARIABLE_SET);
                _context_menu->set_item_metadata(_context_menu->get_item_index(CM_VARIABLE_SET),
                                                 memnew(OrchestratorGraphNodeSpawnerVariableSet(variable_name)));
            }
...
void OrchestratorGraphEdit::_confirm_yes_no(const String& p_text, const String& p_title, Callable p_confirm_callback)
{
    ...
    dialog->connect("close_requested", callable_mp_lambda(this, [dialog] {
                        dialog->queue_free();
                    }));
...
PackedVector2Array OrchestratorGraphEdit::_get_connection_knot_points(const OScriptConnection& p_connection,
                                                                      bool p_apply_zoom) const
{
    PackedVector2Array array;
...
void OrchestratorGraphEdit::_focus_node(int p_node_id, bool p_animated)
{
    if (p_node_id >= 0)
    {
        if (OrchestratorGraphNode* node = _get_node_by_id(p_node_id))
        {
            ...
            if (!VariantUtilityFunctions::is_equal_approx(1.f, get_zoom()))
                tween->tween_method(Callable(this, "set_zoom"), get_zoom(), 1.f, duration);

            Ref<MethodTweener> scroll_tween = tween->tween_method(Callable(this, "set_scroll_offset"),
                                                                  get_scroll_offset(), position, duration);
            if (!VariantUtilityFunctions::is_equal_approx(1.f, get_zoom()))
...
bool OrchestratorGraphEdit::_get_connection_for_points(const Vector2& p_from_position, const Vector2& p_to_position,
...
void OrchestratorGraphEdit::_queue_autowire(const Ref<OScriptNodePin>& p_source, const Ref<OScriptNode>& p_spawned,
                                            const Callable& p_callback)
{
    ...
    _autowire->connect("confirmed",
                       callable_mp(this, &OrchestratorGraphEdit::_complete_autowire).bind(p_spawned, p_callback, true));
    _autowire->connect(
        "canceled", callable_mp(this, &OrchestratorGraphEdit::_complete_autowire).bind(p_spawned, p_callback, false));
...
void OrchestratorGraphEdit::_complete_autowire(const Ref<OScriptNode>& p_spawned, const Callable& p_callback,
                                               bool p_confirmed)
{
    if (p_confirmed)
    {
...
void OrchestratorGraphEdit::_create_script_function_callable(const StringName& p_function_name)
{
    ...
    for (; ctor_index < callable_type.constructors.size(); ++ctor_index)
    {
        ...
        if (ctor.arguments.size() == 2 && ctor.arguments[0].type == Variant::OBJECT
    ...
    spawn_node<OScriptNodeComposeFrom>(
        compose_context, _saved_mouse_position, callable_mp_lambda(this, [=, this](OScriptNodeComposeFrom* compose) {
            ...
            spawn_node<OScriptNodeSelf>(self_context, self_position, callable_mp_lambda(this, [=](OScriptNodeSelf* self) {
...
void OrchestratorGraphEdit::_on_delete_nodes_requested(const PackedStringArray& p_node_names)
{
    ...
    if (!_disable_delete_confirmation && settings->get_setting("ui/graph/confirm_on_delete", true))
    {
        ...
        _confirm_yes_no(message, "Confirm deletion",
                        callable_mp(this, &OrchestratorGraphEdit::_delete_nodes).bind(p_node_names));
    }
    else
    ...
    for (const String& node_name : p_node_names)
    {
        if (OrchestratorGraphNode* node = _get_by_name<OrchestratorGraphNode>(node_name))
        {
            if (!node->get_script_node()->can_user_delete_node())
            {
                ...
                _notify(vformat(message, node->get_script_node_id(), node->get_script_node()->get_node_title()),
                        "Delete canceled");
                return;
            }
        }
...
void OrchestratorGraphEdit::_on_copy_nodes_request()
{
    ...
    for (const Ref<OScriptNode>& node : selected)
    {
        ...
        {
            _notify(vformat("Cannot duplicate node '%s' (%d).", node->get_node_title(), node->get_id()),
                    "Clipboard error");
            return;
        }
    }
...
void OrchestratorGraphEdit::_on_paste_nodes_request()
{
    ...
    for (const Ref<OScriptFunction>& E : _clipboard->functions)
    {
        ...
        {
            _notify(vformat("Function '%s' does not exist in this orchestration.", E->get_function_name()),
                    "Clipboard error");
            return;
        }
        ...
        {
            _notify(vformat("Function '%s' exists with a different definition.", E->get_function_name()),
                    "Clipboard error");
            return;
        }
    }
    ...
    for (const Ref<OScriptVariable>& E : _clipboard->variables)
    {
        ...
        {
            _notify(vformat("Variable '%s' exists with a different definition.", E->get_variable_name()),
                    "Clipboard error");
            return;
        }
    }
