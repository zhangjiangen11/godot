...
void OrchestratorGraphNodePinEnum::_generate_items()
{
    ...
    if (!enum_class.is_empty() && enum_class.contains(".") && enum_class != "Variant.Type")
    {
        ...
        if (enum_class.begins_with("res://"))
        {
            ...
            Ref<Script> script = ResourceLoader::load(class_name);
        ...
        else
        {
            ...
            else if (OrchestratorScriptServer::is_global_class(class_name))
            {
                Dictionary constants = OrchestratorScriptServer::get_global_class(class_name).get_constants_list();
