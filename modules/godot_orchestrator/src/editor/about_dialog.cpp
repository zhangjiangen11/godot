...
void OrchestratorAboutDialog::_notification(int p_what)
{
    if (p_what == NOTIFICATION_READY)
    {
        ...
        about_text->set_text(
            String::utf8("\xc2\xa9 2023-present ") + ("Crater Crash Studios, LLC and it's contributors."));
...
ScrollContainer* OrchestratorAboutDialog::_populate_list(const String& p_name, const List<String>& p_sections,
                                                         ...
                                                         bool p_donor, const bool p_allow_website)
{
    ...
    for (int i = 0; i < p_sections.size(); i++)
    {
        ...
        if (*names_ptr)
        {
            ...
            if (p_allow_website)
            {
                ...
                list->connect("item_activated",
                              callable_mp(this, &OrchestratorAboutDialog::_on_item_website_selected).bind(list));
