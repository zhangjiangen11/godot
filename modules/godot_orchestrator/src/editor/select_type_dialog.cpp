...
PackedStringArray OrchestratorSelectTypeSearchDialog::_get_class_hierarchy(const String& p_class)
{
    ...
    if (OrchestratorScriptServer::is_global_class(p_class))
    {
        hierarchy = OrchestratorScriptServer::get_class_hierarchy(p_class, true);
    }
    else
    {
...
Vector<Ref<OrchestratorEditorSearchDialog::SearchItem>> OrchestratorSelectTypeSearchDialog::_get_search_items()
{
    ...
    // Global Class Types
    for (const String& class_name : OrchestratorScriptServer::get_global_class_list())
    {
    ...
    const String& p_class, HashMap<String, Ref<SearchItem>>& r_cache, const Ref<SearchItem>& p_root)
{
    ...
    for (; class_index < hierarchy.size(); class_index++)
    {
        ...
        if (OrchestratorScriptServer::is_global_class(clazz_name))
            item->script_filename = OrchestratorScriptServer::get_global_class(clazz_name).path.get_file();
...
Vector<Ref<OrchestratorEditorSearchDialog::SearchItem>> OrchestratorSelectTypeSearchDialog::_get_recent_items() const
{
    ...
    const Ref<FileAccess> recents = FileUtils::open_project_settings_file(
        vformat("orchestrator_recent_history.%s", _data_suffix), FileAccess::READ);
...
Vector<Ref<OrchestratorEditorSearchDialog::SearchItem>> OrchestratorSelectTypeSearchDialog::_get_favorite_items() const
{
    ...
    const Ref<FileAccess> recents = FileUtils::open_project_settings_file(
        vformat("orchestrator_favorites.%s", _data_suffix), FileAccess::READ);
    FileUtils::for_each_line(recents, [&](const String& line) {
        if (const String trimmed = line.strip_edges(); !trimmed.is_empty())
        {
...
{
    RBSet<String> recent_items;
    Ref<FileAccess> file = FileUtils::open_project_settings_file(
        vformat("orchestrator_recent_history.%s", _data_suffix), FileAccess::WRITE);
...
{
    Ref<FileAccess> file = FileUtils::open_project_settings_file(vformat("orchestrator_favorites.%s", _data_suffix),
                                                                 FileAccess::WRITE);
...
void OrchestratorSelectTypeSearchDialog::popup_create(bool p_dont_clear, bool p_replace_mode,
