...
struct OScriptUtilityFunctionsDefinitions
{
    ...
    static inline void print_debug(Variant* r_ret, const Variant** p_args, int p_arg_count, GDExtensionCallError& r_error)
    {
        ...
        const uint64_t caller_thread = Thread::get_caller_id();
        if (caller_thread == ::Thread::get_main_id())
        {
            OScriptLanguage* script = OScriptLanguage::get_singleton();
            if (script->debug_get_stack_level_count() > 0)
            {
                s += "\n    At: " + script->_debug_get_stack_level_source(0) + ":"
                     + itos(script->_debug_get_stack_level_line(0)) + ":" + script->_debug_get_stack_level_function(0)
                     + "()";
            }
        }
        else
    ...
    static inline void print_stack(Variant* r_ret, const Variant** p_args, int p_arg_count, GDExtensionCallError& r_error)
    {
        ...
        if (caller_thread != ::Thread::get_main_id())
        {
            print_line("Cannot retrieve debug info outside the main thread. Thread ID: " + itos(caller_thread));
            return;
        }
        ...
        {
            print_line(vformat("Frame %d - %s:%d in function '%s'", i, script->debug_get_stack_level_source(i),
                               script->debug_get_stack_level_line(i), script->debug_get_stack_level_function(i)));
        }
    ...
    static inline void len(Variant* r_ret, const Variant** p_args, int p_arg_count, GDExtensionCallError& r_error)
    {
        ...
        switch (p_args[0]->get_type())
        {
            ...
            default:
            {
                *r_ret = vformat("Value of type '%s' cannot provide a length",
                                 Variant::get_type_name(p_args[0]->get_type()));
                r_error.error = GDEXTENSION_CALL_ERROR_INVALID_ARGUMENT;
                r_error.argument = 0;
...
static void _register_function(const StringName& p_name, const MethodInfo& p_method,
                               OScriptUtilityFunctions::FunctionPtr p_function, bool p_is_const)
{
