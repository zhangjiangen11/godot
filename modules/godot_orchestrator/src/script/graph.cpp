...
void OScriptGraph::_initialize_node(const Ref<OScriptNode>& p_node, const OScriptNodeInitContext& p_context,
...
void OScriptGraph::post_initialize()
{
    ...
    for (const int node_id : _nodes)
    {
        ...
        {
            WARN_PRINT(vformat("Script '%s': Migrating registration in graph %s for node ID %d.",
    ...
    while (iterator != _functions.end())
    {
        ...
        {
            WARN_PRINT(vformat("Script '%s': Removed orphan function reference found in graph %s for node ID %d.",
...
{
    if (!VariantUtilityFunctions::is_equal_approx(_zoom, p_zoom))
    {
        _zoom = p_zoom;
        emit_changed();
    }
...
Ref<OScriptNode> OScriptGraph::create_node(const StringName& p_type, const OScriptNodeInitContext& p_context,
                                           const Vector2& p_position)
{
