...
public:
    ...
    typedef Error (*ParseResourceFunction)(void* p_self, Stream* p_stream, Ref<Resource>& r_res, int& p_line,
                                           String& r_err_str);
...
private:
    ...
    static Error _parse_dictionary(Stream* p_stream, Dictionary& p_object, int& p_line, String& r_err_string,
                                   ResourceParser* p_res_parser = nullptr);
    static Error _parse_array(Stream* p_stream, Array& p_array, int& p_line, String& r_err_string,
                              ResourceParser* p_res_parser = nullptr);
    static Error _parse_tag(Stream* p_stream, Token& p_token, int& p_line, String& r_err_string, Tag& r_tag,
                            ResourceParser* p_res_parser = nullptr, bool p_simple_tag = false);

public:
    static Error parse_tag(Stream* p_stream, int& p_line, Tag& r_tag, String& r_err_string,
                           ResourceParser* p_res_parser = nullptr, bool p_simple_tag = false);
    static Error parse_tag_assign_eof(Stream* p_stream, int& p_line, String& r_err_string, Tag& r_tag, String& r_assign,
                                      Variant& r_value, ResourceParser* p_res_parser = nullptr,
                                      bool p_simple_tag = false);
    static Error parse_value(Stream* p_stream, Token& p_token, int& p_line, Variant& r_value, String& r_err_string,
                             ResourceParser* p_res_parser = nullptr);
    static Error parse(Stream* p_stream, Variant& r_ret, String& r_err_string, int& r_err_line,
                       ResourceParser* p_res_parser = nullptr);
...
public:
    ...
    static Error write(const Variant& p_variant, StoreStringFunction p_store_string, void* p_store_userdata,
                       EncodeResourceFunction p_encode_resource, void* p_encode_userdata, int p_recursion_count = 0);
    static Error write_to_string(const Variant& p_variant, String& r_string,
                                 EncodeResourceFunction p_encode_resource = nullptr, void* p_encode_userdata = nullptr);
};
