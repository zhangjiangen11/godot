...
#define _printerr() \
    ERR_PRINT(String(_res_path + ":" + itos(_lines) + " - Parse Error: " + _error_text).utf8().get_data());
...
Error OScriptTextResourceLoaderInstance::_parse_sub_resources(void* p_self, OScriptVariantParser::Stream* p_stream,
                                                              Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
    return reinterpret_cast<OScriptTextResourceLoaderInstance*>(p_self)->_parse_sub_resource(p_stream, r_res, r_line,
                                                                                             r_err_string);
}

Error OScriptTextResourceLoaderInstance::_parse_sub_resource_dummys(
    void* p_self, OScriptVariantParser::Stream* p_stream, Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
    return _parse_sub_resource_dummy(reinterpret_cast<DummyReadData*>(p_self), p_stream, r_res, r_line, r_err_string);
}

Error OScriptTextResourceLoaderInstance::_parse_sub_resource_dummy(
    DummyReadData* p_data, OScriptVariantParser::Stream* p_stream, Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
...
Error OScriptTextResourceLoaderInstance::_parse_sub_resource(OScriptVariantParser::Stream* p_stream,
                                                             Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
    OScriptVariantParser::Token token;
    OScriptVariantParser::get_token(p_stream, r_line, token, r_err_string);
...
Error OScriptTextResourceLoaderInstance::_parse_ext_resources(void* p_self, OScriptVariantParser::Stream* p_stream,
                                                              Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
    return reinterpret_cast<OScriptTextResourceLoaderInstance*>(p_self)->_parse_ext_resource(p_stream, r_res, r_line,
                                                                                             r_err_string);
}

Error OScriptTextResourceLoaderInstance::_parse_ext_resource_dummys(
    void* p_self, OScriptVariantParser::Stream* p_stream, Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
    return _parse_ext_resource_dummy(reinterpret_cast<DummyReadData*>(p_self), p_stream, r_res, r_line, r_err_string);
}

Error OScriptTextResourceLoaderInstance::_parse_ext_resource_dummy(
    DummyReadData* p_data, OScriptVariantParser::Stream* p_stream, Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
    OScriptVariantParser::Token token;
    OScriptVariantParser::get_token(p_stream, r_line, token, r_err_string);
...
Error OScriptTextResourceLoaderInstance::_parse_ext_resource(OScriptVariantParser::Stream* p_stream,
                                                             Ref<Resource>& r_res, int& r_line, String& r_err_string)
{
    OScriptVariantParser::Token token;
    OScriptVariantParser::get_token(p_stream, r_line, token, r_err_string);
...
PackedStringArray OScriptTextResourceLoaderInstance::get_dependencies(const Ref<FileAccess>& p_file, bool p_add_types)
{
    ...
    while (_next_tag.name == "ext_resource")
    {
        ...
        if (err)
        {
            print_line(_error_text + " - " + itos(_lines));
...
Error OScriptTextResourceLoaderInstance::rename_dependencies(const Ref<FileAccess>& p_file, const String& p_path,
                                                             const Dictionary& p_renames)
{
    open(p_file, true);
...
Error OScriptTextResourceLoaderInstance::load()
{
    ...
    while (true)
    {
        ...
        if (_next_tag.fields.has("uid"))
        {
            ...
            else
            {
                ...
                {
                    WARN_PRINT(String(_res_path + ":" + itos(_lines) + " - ext_resource, invalid UID: " + uidt
                                      + " - using text path instead: " + path)
                                   .utf8()
                                   .get_data());
                }
#else
        ...
        _external_resources[id].resource = ResourceLoader::load(path, type, (ResourceLoader::CacheMode)_cache_mode);
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            _error = OScriptVariantParser::parse_tag_assign_eof(&_stream, _lines, _error_text, _next_tag, assign, value,
                                                                &_rp);
            if (_error)
            {
                _printerr();
                    ...
                    if (value.get_type() == Variant::ARRAY)
                    {
                        ...
                        if (get_value.get_type() == Variant::ARRAY)
                        {
                            ...
                            if (!set_array.is_same_typed(get_array))
                                value = Array(set_array, get_array.get_typed_builtin(),
                                              get_array.get_typed_class_name(), get_array.get_typed_script());
                        }
                    }
