            ...
            switch (obj_type)
            {
                ...
                case OBJECT_EXTERNAL_RESOURCE:
                {
                    ...
                    if (!path.contains("://") && path.is_relative_path())
                    {
                        ...
                        path = ProjectSettings::get_singleton()->localize_path(
                            _resource_path.get_base_dir().path_join(path));
                    }

                    if (_remaps.has(path))
                        path = _remaps[path];

                    Ref<Resource> res = ResourceLoader::load(path, ext_type, (ResourceLoader::CacheMode)_cache_mode_ext);
                ...
                case OBJECT_EXTERNAL_RESOURCE_INDEX:
                {
                    ...
                    else
                    {
                        ...
                        Ref<Resource> res = ResourceLoader::load(_external_resources[err_index].path,
                                                                 _external_resources[err_index].type);
                        if (res.is_null())
                        {
                            ERR_FAIL_V_MSG(ERR_FILE_MISSING_DEPENDENCIES,
                                           "Cannot load dependency: " + _external_resources[err_index].path + ".");
                        }
                        r_val = res;
                    }
                    break;
...
Error OScriptBinaryResourceLoaderInstance::rename_dependencies(const Ref<FileAccess>& p_file, const String& p_path,
...
void OScriptBinaryResourceLoaderInstance::open(const Ref<FileAccess>& p_file, bool p_no_resources, bool p_keep_uuid_paths)
{
    ...
    if (_version > FORMAT_VERSION)
    {
        ...
        ERR_FAIL_MSG(vformat(
            ...
            _local_path, _version, FORMAT_VERSION));
    }
    ...
    for (uint32_t i = 0; i < RESERVED_FIELDS; i++) [[maybe_unused]]
        uint32_t x = _file->get_32();
    ...
    if (_version >= 3)
    {
        ...
        for (uint32_t i = 0; i < external_resource_count; i++)
        {
            ...
            if (!p_keep_uuid_paths && er.uid != ResourceUID::INVALID_ID)
            {
                ...
                {
#ifdef TOOLS_ENABLED
                    if (ResourceLoader::get_resource_uid(_resource_path) != er.uid)
                    {
...
Error OScriptBinaryResourceLoaderInstance::load()
{
    ...
    for (int i = 0; i < _external_resources.size(); i++)
    {
        ...
        if (!path.contains("://") && path.is_relative_path())
        {
            ...
            path = ProjectSettings::get_singleton()->localize_path(
                path.get_base_dir().path_join(_external_resources[i].path));
        }
    ...
    for (int i = 0; i < _internal_resources.size(); i++)
    {
        ...
        if (res.is_null())
        {
            ...
            if (!r)
            {
                ...
                _error = ERR_FILE_CORRUPT;
                ERR_FAIL_V_MSG(_error,
                               _local_path + ": Resource type is resource field not a resource, type is: " + obj_class);
            }
