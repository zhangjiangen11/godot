    ...
    if (!_resources.is_empty())
    {
        if (OS::get_singleton()->is_stdout_verbose())
        {
            ...
            for (const KeyValue<String, Resource*>& E : _resources)
                print_line("Resource still in use: ", E.key, " (", E.value->get_class(), ")");
        }
        else
        {
            ERR_PRINT(vformat("%d resources still in use at exit (Orchestrator) (run with --verbose for details)",
                              _resources.size()));
        }
    }
    _resources.clear();
...
void ResourceCache::set_scene_unique_id(const String& p_path, const Ref<Resource>& p_resource, const String& p_id)
{
    ...
    Variant weak_ref = VariantUtilityFunctions::weakref(p_resource);
