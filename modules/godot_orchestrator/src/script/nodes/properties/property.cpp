...
TypedArray<Dictionary> OScriptNodeProperty::_get_class_property_list(const String& p_class_name) const
{
    if (OrchestratorScriptServer::is_global_class(p_class_name))
        return OrchestratorScriptServer::get_global_class(p_class_name).get_property_list();
...
void OScriptNodeProperty::validate_node_during_build(BuildLog& p_log) const
{
    if (_call_mode == CALL_INSTANCE)
    {
        ...
        //             p_log.error(this, vformat("No property name '%s' found in class '%s'", _property.name,
    ...
    else if (_call_mode == CALL_NODE_PATH)
    {
        ...
        //
        //         if (!script_property && !object_property)
        //             p_log.error(this, vformat("No property name '%s' found for node path '%s'.", _property.name,
        //             _node_path));
        //     }
        // }
    }
