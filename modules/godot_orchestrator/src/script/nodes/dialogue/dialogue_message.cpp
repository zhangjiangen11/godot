...
public:
    ...
    int step(OScriptExecutionContext& p_context) override
    {
        ...
        Ref<PackedScene> scene = ResourceLoader::load(scene_file);
        if (scene.is_valid())
        {
...
void OScriptNodeDialogueMessage::allocate_default_pins()
{
    ...
    {
        for (int i = 0; i < 4; i++)
            create_pin(PD_Output, PT_Execution, PropertyUtils::make_exec("temp_" + itos(i)))
                ->set_flag(OScriptNodePin::Flags::HIDDEN);
