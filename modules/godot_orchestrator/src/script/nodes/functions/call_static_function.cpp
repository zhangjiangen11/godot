...
class OScriptNodeCallStaticFunctionInstance : public OScriptNodeInstance
{
    ...
    int _call_native_method(OScriptExecutionContext& p_context, const Array& p_args)
    {
        ...
        internal::gdextension_interface_object_method_bind_call(
            mb, nullptr, reinterpret_cast<GDExtensionConstVariantPtr*>(call_args.data()), call_args.size(), &ret,
            &r_error);
...
void OScriptNodeCallStaticFunction::_resolve_method_info()
{
    ...
    if (OrchestratorScriptServer::is_global_class(_class_name))
        methods = OrchestratorScriptServer::get_global_class(_class_name).get_method_list();
    else
...
void OScriptNodeCallStaticFunction::allocate_default_pins()
{
    ...
    for (size_t arg_index = 0; arg_index < _method.arguments.size(); arg_index++)
    {
        ...
        const Variant default_value = arg_index >= default_start_index ? _method.default_arguments[def_index++] :
                                                                         Variant();
    ...
    if (MethodUtils::has_return_value(_method))
    {
        Ref<OScriptNodePin> rvalue = create_pin(PD_Output, PT_Data,
                                                PropertyUtils::as("return_value", _method.return_val));
...
OScriptNodeInstance* OScriptNodeCallStaticFunction::instantiate()
{
    ...
    if (OrchestratorScriptServer::is_global_class(_class_name))
    {
        const OrchestratorScriptServer::GlobalClass gc = OrchestratorScriptServer::get_global_class(_class_name);
        if (!gc.path.is_empty())
        {
            Ref<Script> script = ResourceLoader::load(gc.path, "Script");
