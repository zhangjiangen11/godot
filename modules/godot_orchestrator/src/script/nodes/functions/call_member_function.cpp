        ...
        if (target.is_valid() && PropertyUtils::is_nil_no_variant(target->get_property_info()))
        {
            ...
            if (target->has_any_connections())
            {
                ...
                else
                {
                    ...
                    const String class_name = _get_method_class_hierarchy_owner(_reference.target_class_name,
                                                                                _reference.method.name);
            ...
            else
            {
                ...
                const String class_name = _get_method_class_hierarchy_owner(get_orchestration()->get_base_type(),
                                                                            _reference.method.name);
                if (!class_name.is_empty())
                {
                    _reference.target_class_name = class_name;
...
StringName OScriptNodeCallMemberFunction::_get_method_class_hierarchy_owner(const String& p_class_name,
                                                                            const String& p_method_name)
{
    ...
    while (!class_name.is_empty())
    {
        ...
        if (OrchestratorScriptServer::is_global_class(class_name))
            methods = OrchestratorScriptServer::get_global_class(class_name).get_method_list();
        else
