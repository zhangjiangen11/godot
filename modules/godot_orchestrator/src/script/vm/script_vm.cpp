...
void OScriptVirtualMachine::_set_unassigned_inputs(const Ref<OScriptNode>& p_node,
                                                   const OScriptNodeInstance* p_instance, Function& r_function)
{
...
void OScriptVirtualMachine::_set_unassigned_outputs(const Ref<OScriptNode>& p_node,
                                                    const OScriptNodeInstance* p_instance, int p_stack_pos)
{
...
void OScriptVirtualMachine::_get_execution_path(Orchestration* p_orchestration, int p_node_id,
                                                RBSet<OScriptConnection>& r_connections, RBSet<int>& r_execution_path)
{
...
void OScriptVirtualMachine::_get_data_connection_lookup(Orchestration* p_orchestration,
                                                        HashMap<int, HashMap<int, Pair<int, int>>>& r_lookup)
{
    for (const OScriptConnection& E : p_orchestration->get_connections())
    {
...
bool OScriptVirtualMachine::_create_node_instance(Orchestration* p_orchestration, int p_node_id, Function& r_function,
                                                  HashMap<String, int>& r_lv_indices)
{
    ...
    if (lv_node.is_valid() || assign_lv_node.is_valid())
    {
        ...
        const String name = assign_lv_node.is_valid() ? assign_lv_node->get_variable_guid() :
                                                        lv_node->get_variable_guid();
...
bool OScriptVirtualMachine::_build_function_node_graph(const Ref<OScriptFunction>& p_function, Function& r_function,
                                                       HashMap<String, int>& r_lv_indices)
{
    ...
    for (const int E : execution_path)
    {
        if (!_create_node_instance(p_function->get_orchestration(), E, r_function, r_lv_indices))
        {
            OScriptLanguage::get_singleton()->debug_break_parse(
                p_function->get_orchestration()->get_self()->get_path(), 0,
                "Failed to create function node instance for node with ID " + itos(E) + " for function "
                    + p_function->get_function_name());
...
void OScriptVirtualMachine::_resolve_inputs(OScriptExecutionContext& p_context, OScriptNodeInstance* p_instance,
                                            Function* p_function)
{
...
void OScriptVirtualMachine::_copy_stack_to_node_outputs(OScriptExecutionContext& p_context,
                                                        OScriptNodeInstance* p_instance)
{
    for (int i = 0; i < p_instance->data_output_pin_count; i++)
...
void OScriptVirtualMachine::_dependency_step(OScriptExecutionContext& p_context, OScriptNodeInstance* p_instance,
...
int OScriptVirtualMachine::_execute_step(OScriptExecutionContext& p_context, OScriptNodeInstance* p_instance)
{
    ...
    if (debugger && debugger->is_active())
    {
        ...
        if (do_break && !debugger->is_skipping_breakpoints())
            OScriptLanguage::get_singleton()->debug_break("Breakpoint: Before Node " + itos(node_id) + " executes.",
                                                          true);
...
OScriptNodeInstance* OScriptVirtualMachine::_resolve_next_node(
    OScriptExecutionContext& p_context, OScriptNodeInstance* p_instance, int p_result, int p_next_node_id)
{
    if ((p_result == p_next_node_id || p_result & OScriptNodeInstance::STEP_FLAG_PUSH_STACK_BIT)
        && p_instance->execution_output_pin_count)
    {
        ...
        p_context.set_error(vformat("Node %s: %d returned an invalid execution pin output %d",
                                    p_instance->get_base_node()->get_class(), p_instance->get_id(), p_next_node_id));
    }

    return nullptr;
}

int OScriptVirtualMachine::_resolve_next_node_port(OScriptNodeInstance* p_instance, int p_next_node_id,
                                                   OScriptNodeInstance* p_next)
{
...
void OScriptVirtualMachine::_set_node_flow_execution_state(OScriptExecutionContext& p_context,
                                                           OScriptNodeInstance* p_instance, int p_result)
{
...
void OScriptVirtualMachine::_report_error(OScriptExecutionContext& p_context, OScriptNodeInstance* p_instance,
...
void OScriptVirtualMachine::_call_method_internal(const StringName& p_method, OScriptExecutionContext* p_context,
                                                  bool p_resume, OScriptNodeInstance* p_instance, Function* p_function,
                                                  Variant& r_return, GDExtensionCallError& r_err)
{
    ...
    while (node)
    {
        ...
        {
            ERR_PRINT(vformat("Script call error #%d with Node %d: %s", context.get_error().error,
                              context.get_current_node(), context.get_error_reason()));
        ...
        if (EngineDebugger::get_singleton()->is_active())
        {
            ...
            if (do_break && !EngineDebugger::get_singleton()->is_skipping_breakpoints())
                OScriptLanguage::get_singleton()->debug_break(
                    "Breakpoint: After Node " + itos(node_id) + " has executed.", true);
...
bool OScriptVirtualMachine::register_variable(const Ref<OScriptVariable>& p_variable)
{
    ...
    ERR_FAIL_COND_V_MSG(_variables.has(p_variable->get_variable_name()), false,
                        "A variable is defined with the name: " + p_variable->get_variable_name());
...
void OScriptVirtualMachine::call_method(OScriptInstance* p_instance, const StringName& p_method,
                                        const Variant* const* p_args, GDExtensionInt p_arg_count, Variant* r_return,
