...
protected:
    ...
    _FORCE_INLINE_ void _copy_stack_to_input(int p_stack_index, int p_input_index)
    {
        _inputs[p_input_index] = &_variant_stack[p_stack_index];
    }
    ...
    _FORCE_INLINE_ void _copy_stack_to_output(int p_stack_index, int p_output_index)
    {
        _outputs[p_output_index] = &_variant_stack[p_stack_index];
    }
    ...
    _FORCE_INLINE_ void _push_node_onto_flow_stack(int p_node_id)
    {
        if (_flow_stack)
            _flow_stack[0] = p_node_id;
    }
    //~ End Flow Stack Interface

    //~ Begin Flow Stack Bit Interface
    _FORCE_INLINE_ bool _has_flow_stack_bit(int p_bit)
    {
        return _has_flow_stack() && _flow_stack[_flow_stack_position] & p_bit;
    }
...
public:
    ...
    _FORCE_INLINE_ Variant get_working_memory(int p_index = 0)
    {
        static Variant empty = Variant();
        return has_working_memory() ? _working_memory[p_index] : empty;
    }
    _FORCE_INLINE_ void set_working_memory(int p_index)
    {
        _working_memory = p_index >= 0 ? &(_variant_stack[p_index]) : nullptr;
    }
    ...
    _FORCE_INLINE_ void set_input(int p_index, const Variant* p_value)
    {
        _inputs[p_index] = const_cast<Variant*>(p_value);
    }
    ...
    _FORCE_INLINE_ bool set_output(int p_index, const Variant& p_value)
    {
        *_outputs[p_index] = p_value;
        return true;
    }
    _FORCE_INLINE_ bool set_output(int p_index, Variant* p_value)
    {
        *_outputs[p_index] = *p_value;
        return true;
    }
